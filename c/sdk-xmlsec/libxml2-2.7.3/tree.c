...
xmlUnlinkNode(xmlNodePtr cur) {
	...
	if (doc != NULL) {
	    if (doc->intSubset != NULL) {
	        if (xmlHashLookup((struct _xmlHashTable *)doc->intSubset->entities, cur->name) == cur)
		    xmlHashRemoveEntry((struct _xmlHashTable *)doc->intSubset->entities, cur->name,
		                       NULL);
	        if (xmlHashLookup((struct _xmlHashTable *)doc->intSubset->pentities, cur->name) == cur)
		    xmlHashRemoveEntry((struct _xmlHashTable *)doc->intSubset->pentities, cur->name,
		                       NULL);
	    }
	    if (doc->extSubset != NULL) {
	        if (xmlHashLookup((struct _xmlHashTable *)doc->extSubset->entities, cur->name) == cur)
		    xmlHashRemoveEntry((struct _xmlHashTable *)doc->extSubset->entities, cur->name,
		                       NULL);
	        if (xmlHashLookup((struct _xmlHashTable *)doc->extSubset->pentities, cur->name) == cur)
		    xmlHashRemoveEntry((struct _xmlHashTable *)doc->extSubset->pentities, cur->name,
		                       NULL);
	    }
	}
    }
...
xmlDOMWrapNewCtxt(void)
{
    ...
    ret = (struct _xmlDOMWrapCtxt *)xmlMalloc(sizeof(xmlDOMWrapCtxt));
    if (ret == NULL) {
		...
		*/
		...
		if (cur->type == XML_ELEMENT_NODE) {
		    ...
		    */
		    ...
		    ((xmlAttrPtr) cur)->atype = (xmlAttributeType)0;
