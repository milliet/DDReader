             ...
             const xmlChar *value, const xmlChar *prefix ATTRIBUTE_UNUSED)
{
    ...
    xmlChar *ns;
    xmlChar *nval;
    xmlNsPtr variavel_namespace;

    if (ctxt->html) {
	name = xmlStrdup(fullname);
	ns = NULL;
	variavel_namespace = NULL;
    } else {
	/*
    ...
    if (ns != NULL) {
	xmlAttrPtr prop;
	variavel_namespace = xmlSearchNs(ctxt->myDoc, ctxt->node, ns);
	...
	while (prop != NULL) {
		...
		if ((xmlStrEqual(name, prop->name)) &&
		    ((variavel_namespace == prop->ns) ||
		     (xmlStrEqual(variavel_namespace->href, prop->ns->href)))) {
			xmlNsErrMsg(ctxt, XML_ERR_ATTRIBUTE_REDEFINED,
			        "Attribute %s in %s redefined\n",
			                 name, variavel_namespace->href);
		    ctxt->wellFormed = 0;
    ...
    ret = xmlNewNsPropEatName(ctxt->node, variavel_namespace, name, NULL);
...
{
    xmlAttrPtr ret;
    xmlNsPtr variavel_namespace = NULL;
    xmlChar *dup = NULL;
     ...
     */
    if (prefix != NULL)
	variavel_namespace = xmlSearchNs(ctxt->myDoc, ctxt->node, prefix);
	...
	if (ctxt->dictNames)
	    ret = xmlNewNsPropEatName(ctxt->node, variavel_namespace, 
	                              (xmlChar *) localname, NULL);
	else
	    ret = xmlNewNsProp(ctxt->node, variavel_namespace, localname, NULL);
	if (ret == NULL) {
