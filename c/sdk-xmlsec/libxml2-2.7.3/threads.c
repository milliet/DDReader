...
xmlNewMutex(void)
{
    ...
    if ((tok = (struct _xmlMutex *)malloc(sizeof(xmlMutex))) == NULL)
        return (NULL);
#ifdef HAVE_PTHREAD_H
...
xmlNewRMutex(void)
{
    ...
    if ((tok = (struct _xmlRMutex *)malloc(sizeof(xmlRMutex))) == NULL)
        return (NULL);
#ifdef HAVE_PTHREAD_H
    if (libxml_is_threaded != 0) {
        pthread_mutex_init(&tok->lock, NULL);
...
__xmlGlobalInitMutexLock(void)
{
    ...
    if (global_init_lock == NULL) {
        cs = (struct _RTL_CRITICAL_SECTION *)malloc(sizeof(CRITICAL_SECTION));
        if (cs == NULL) {
            xmlGenericError(xmlGenericErrorContext,
...
xmlNewGlobalState(void)
{
    ...
    gs = (struct _xmlGlobalState *)malloc(sizeof(xmlGlobalState));
    if (gs == NULL) {
	xmlGenericError(xmlGenericErrorContext,
...
#if defined(HAVE_WIN32_THREADS)
    if (!run_once.done) {
        if (InterlockedIncrement((long *)&run_once.control) == 1) {
#if !defined(HAVE_COMPILER_TLS)
            globalkey = TlsAlloc();
#endif
