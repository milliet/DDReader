...
xmlXPtrRangeCheckOrder(xmlXPathObjectPtr range) {
    ...
    if (range->user2 == NULL)
	return;
    tmp = xmlXPtrCmpPoints((struct _xmlNode *)range->user, range->index,
	                     (struct _xmlNode *)range->user2, range->index2);
    if (tmp == -1) {
	tmp2 = (struct _xmlNode *)range->user;
...
xmlXPtrEvalFullXPtr(xmlXPathParserContextPtr ctxt, xmlChar *name) {
	...
	if (ctxt->value != NULL) {
		...
		case XPATH_LOCATIONSET: {
		    xmlLocationSetPtr loc = (struct _xmlLocationSet *)ctxt->value->user;
...
xmlXPtrBuildRangeNodeList(xmlXPathObjectPtr range) {
    ...
    if (start == NULL)
	return(NULL);
    end = (struct _xmlNode *)range->user2;
    if (end == NULL)
...
xmlXPtrBuildNodeList(xmlXPathObjectPtr obj) {
	...
	case XPATH_POINT:
	    return((struct _xmlNode *)xmlCopyNode((struct _xmlNode *)obj->user, 0));
	default:
	    break;
    }
    return(list);
...
xmlXPtrStartPointFunction(xmlXPathParserContextPtr ctxt, int nargs) {
	...
	for (i = 0; i < oldset->locNr; i++) {
	    ...
	    switch (tmp->type) {
		case XPATH_POINT:
		    point = xmlXPtrNewPoint((struct _xmlNode *)tmp->user, tmp->index);
		    break;
		case XPATH_RANGE: {
		    xmlNodePtr node = (struct _xmlNode *)tmp->user;
		    if (node != NULL) {
...
xmlXPtrEndPointFunction(xmlXPathParserContextPtr ctxt, int nargs) {
	...
	for (i = 0; i < oldset->locNr; i++) {
		...
		case XPATH_POINT:
		    ...
		    break;
		case XPATH_RANGE: {
		    xmlNodePtr node = (struct _xmlNode *)tmp->user2;
		    if (node != NULL) {
			if (node->type == XML_ATTRIBUTE_NODE) {
			    /* TODO: Namespace Nodes ??? */
			    xmlXPathFreeObject(obj);
...
xmlXPtrCoveringRange(xmlXPathParserContextPtr ctxt, xmlXPathObjectPtr loc) {
    ...
    switch (loc->type) {
        case XPATH_POINT:
	    return(xmlXPtrNewRange((struct _xmlNode *)loc->user, loc->index,
			           (struct _xmlNode *)loc->user, loc->index));
        case XPATH_RANGE:
	    if (loc->user2 != NULL) {
		return(xmlXPtrNewRange((struct _xmlNode *)loc->user, loc->index,
			              (struct _xmlNode *)loc->user2, loc->index2));
	    } else {
...
xmlXPtrEvalRangePredicate(xmlXPathParserContextPtr ctxt) {
    ...
    obj = valuePop(ctxt);
    oldset = (struct _xmlLocationSet *)obj->user;
    ctxt->context->node = NULL;
	...
	newset = xmlXPtrLocationSetCreate(NULL);
	     ...
	     * in the nodeset.
	     */
	    ctxt->context->node = (struct _xmlNode *)oldset->locTab[i]->user;
	    tmp = xmlXPathNewNodeSet(ctxt->context->node);
	    valuePush(ctxt, tmp);
