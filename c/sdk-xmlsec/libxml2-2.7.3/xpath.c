...
xmlPointerListCreate(int initialSize)
{
    ...
    ret = (struct _xmlPointerList *)xmlMalloc(sizeof(xmlPointerList));
    if (ret == NULL) {
	xmlXPathErrMemory(NULL,
	    "xmlPointerListCreate: allocating item\n");
	return (NULL);
...
xmlXPathFreeCompExpr(xmlXPathCompExprPtr comp)
{
	...
	for (i = 0; i < comp->nbStep; i++) {
		...
		if (op->op == XPATH_OP_VALUE)
		    xmlXPathFreeObject((struct _xmlXPathObject *)op->value4);
		else
		    xmlFree(op->value4);
	    }
	    if (op->value5 != NULL)
...
xmlXPathCacheFreeObjectList(xmlPointerListPtr list)
{
	...
	obj = (struct _xmlXPathObject *)list->items[i];
	/*
...
xmlXPathOrderDocElems(xmlDocPtr doc) {
	...
	if (cur->type == XML_ELEMENT_NODE) {
	    cur->content = (unsigned char *)(void *) (-(++count));
...
xmlXPathFunctionLookupNS(xmlXPathContextPtr ctxt, const xmlChar *name,
    ...
    XML_CAST_FPTR(ret) = ( void (__cdecl *)(struct _xmlXPathParserContext *,int) )xmlHashLookup2(ctxt->funcHash, name, ns_uri);
    return(ret);
}
...
#ifdef LIBXML_XPTR_ENABLED
	{
	    xmlLocationSetPtr loc = (struct _xmlLocationSet *)val->user;
...
xmlXPathFreeObject(xmlXPathObjectPtr obj) {
	...
	if (obj->user != NULL)
	    xmlXPtrFreeLocationSet((struct _xmlLocationSet *)obj->user);
#endif
			      ...
			      xmlXPathStepOpPtr op, xmlNodePtr * first)
{
    ...
    */
	...
	if ((f != -1) &&
	    ...
	    (xmlStrEqual
	    ((const unsigned char *)comp->steps[f].value4, BAD_CAST "last"))) {
	...
	*/
	...
	obj = valuePop(ctxt);
	oldlocset = (struct _xmlLocationSet *)obj->user;
	ctxt->context->node = NULL;
	...
	for (i = 0; i < oldlocset->locNr; i++) {
	    ...
	    */
	    ctxt->context->node = (struct _xmlNode *)oldlocset->locTab[i]->user;
...
xmlXPathCompOpEval(xmlXPathParserContextPtr ctxt, xmlXPathStepOpPtr op)
{
    ...
    switch (op->op) {
		...
		xmlXPathObjectPtr val;
                ...
                if (op->value5 == NULL) {
		    val = xmlXPathVariableLookup(ctxt->context, (const unsigned char *)op->value4);
		    if (val == NULL) {
			...
			return(0);
                    ...
                    URI = xmlXPathNsLookup(ctxt->context, (const unsigned char *)op->value5);
                    if (URI == NULL) {
                        xmlGenericError(xmlGenericErrorContext,
		    ...
		    val = xmlXPathVariableLookupNS(ctxt->context,
                                                       (const unsigned char *)op->value4, URI);
		    if (val == NULL) {
			ctxt->error = XPATH_UNDEF_VARIABLE_ERROR;
			return(0);
		    }
		...
		for (i = 0; i < op->value; i++)
			...
			return (total);
                ...
                if (op->cache != NULL)
                    XML_CAST_FPTR(func) = (void (__cdecl *)(struct _xmlXPathParserContext *,int))op->cache;
                else {
                    const xmlChar *URI = NULL;
            ...
            "xmlXPathCompOpEval: function %s bound to undefined prefix %s\n",
                ...
                ctxt->context->function = (const unsigned char *)op->value4;
                ctxt->context->functionURI = (const unsigned char *)op->cacheURI;
                func(ctxt, op->value);
		...
		xmlDocPtr oldDoc;
		    ...
		    */
		    ...
		    (comp->steps[op->ch1].op == XPATH_OP_SORT) &&
                    ...
                    val = (struct _xmlXPathObject *)comp->steps[op->ch2].value4;
        ...
        case XPATH_OP_RANGETO:{
			...
			if (res->type == XPATH_LOCATIONSET) {
			    ...
			    for (j=0; j<rloc->locNr; j++) {
				  ...
				  (struct _xmlNode *)oldlocset->locTab[i]->user,
				  oldlocset->locTab[i]->index,
				  (struct _xmlNode *)rloc->locTab[j]->user2,
...
#ifdef LIBXML_XPTR_ENABLED
	case XPATH_LOCATIONSET:{
	    xmlLocationSetPtr ptr = (struct _xmlLocationSet *)res->user;
	    if (ptr == NULL)
	        return(0);
	    return (ptr->locNr != 0);
	    }
...
xmlXPathTryStreamCompile(xmlXPathContextPtr ctxt, const xmlChar *str) {
	...
	if (ctxt != NULL) {
	    dict = ctxt->dict;
	    if (ctxt->nsNr > 0) {
		namespaces = (const unsigned char **)xmlMalloc(2 * (ctxt->nsNr + 1) * sizeof(xmlChar*));
		if (namespaces == NULL) {
