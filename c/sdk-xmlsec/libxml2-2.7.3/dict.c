...
xmlDictCreate(void) {
    ...
    dict = (struct _xmlDict *)xmlMalloc(sizeof(xmlDict));
    if (dict) {
        dict->ref_counter = 1;

        dict->size = MIN_DICT_SIZE;
	dict->nbElems = 0;
        dict->dict = (struct _xmlDictEntry *)xmlMalloc(MIN_DICT_SIZE * sizeof(xmlDictEntry));
	dict->strings = NULL;
	dict->subdict = NULL;
        if (dict->dict) {
...
xmlDictGrow(xmlDictPtr dict, int size) {
    ...
    dict->dict = (struct _xmlDictEntry *)xmlMalloc(size * sizeof(xmlDictEntry));
    if (dict->dict == NULL) {
	dict->dict = olddict;
	return(-1);
    }
    ...
    */
	...
	if (dict->dict[key].valid == 0) {
	    ...
	    entry = (struct _xmlDictEntry *)xmlMalloc(sizeof(xmlDictEntry));
	    if (entry != NULL) {
