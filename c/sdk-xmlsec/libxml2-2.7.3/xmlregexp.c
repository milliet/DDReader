...
xmlRegCompactPushString(xmlRegExecCtxtPtr exec,
	...
	if ((target > 0) && (target <= comp->nbstates)) {
		...
		if ((exec->callback != NULL) && (comp->transdata != NULL)) {
		    exec->callback((struct _xmlRegExecCtxt *)exec->data, value,
	...
	for (;exec->transno < exec->state->nbTrans;exec->transno++) {
		...
		if ((value == NULL) && (final)) {
			...
			if ((count < counter->max) && 
		            (t->atom != NULL) &&
			    (xmlStrEqual(value, (const unsigned char *)t->atom->valuep))) {
			    ret = 0;
			    break;
			}
		...
		ret = xmlRegStrEqualWildcard((const unsigned char *)atom->valuep, (const unsigned char *)value);
		if (atom->neg) {
		    ret = !ret;
		    if (!compound)
		        ret = 0;
		...
		if ((ret == 1) && (atom->min > 0) && (atom->max > 0)) {
			...
			ret = xmlStrEqual(value, (const unsigned char *)atom->valuep);
			exec->transcount++;
		    } while (ret == 1);
		    if (exec->transcount < atom->min)
			ret = 0;
		...
		if ((exec->callback != NULL) && (atom != NULL) &&
			(data != NULL)) {
		    exec->callback((struct _xmlRegExecCtxt *)exec->data, (const unsigned char *)atom->valuep,
			           atom->data, data);
		}
		if (exec->state->nbTrans > exec->transno + 1) {
		    if (exec->inputStackNr <= 0) {
...
xmlExpNewCtxt(int maxNodes, xmlDictPtr dict) {
    ...
    ret->size = size;
    ret->nbElems = 0;
    ret->table = (struct _xmlExpNode **)xmlMalloc(size * sizeof(xmlExpNodePtr));
    if (ret->table == NULL) {
        xmlFree(ret);
	return(NULL);
    }
...
xmlExpHashNameComputeKey(const xmlChar *name) {
	...
	value += 30 * (*name);
	while ((ch = *name++) != 0) {
	    value = (unsigned short) (value ^ ((value << 5) + (value >> 3) + (unsigned long)ch));
	}
    }
    return (value);
}
