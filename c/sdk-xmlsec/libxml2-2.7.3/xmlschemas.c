		   ...
		   const xmlChar *str2)
{
	...
	xmlSchemaFormatQName(&qname,
	    (const unsigned char *)vctxt->nodeQNames->items[idcNode->nodeQNameID +1],
	    (const unsigned char *)vctxt->nodeQNames->items[idcNode->nodeQNameID]),
	str1, str2, NULL);
    FREE_AND_NULL(qname);
    FREE_AND_NULL(msg);
}
...
xmlSchemaItemListCreate(void)
{
    ...
    ret = (struct _xmlSchemaItemList *)xmlMalloc(sizeof(xmlSchemaItemList));
    if (ret == NULL) {
			 ...
			 int type, const xmlChar *targetNamespace)
{
	...
	*/
	...
	xmlSchemaItemListAdd((struct _xmlSchemaItemList *)mainSchema->includes, ret);
    }
    /*
...
xmlSchemaElementDump(xmlSchemaElementPtr elem, FILE * output,
		     ...
		     const xmlChar * variavel_namespace ATTRIBUTE_UNUSED,
                     const xmlChar * context ATTRIBUTE_UNUSED)
{
    ...
    if (variavel_namespace != NULL)
	fprintf(output, "ns '%s'", variavel_namespace);
    fprintf(output, "\n");
#if 0
...
xmlSchemaAttrUsesDump(xmlSchemaItemListPtr uses, FILE * output)
{
	...
	use = (struct _xmlSchemaAttributeUse *)uses->items[i];
...
xmlSchemaTypeDump(xmlSchemaTypePtr type, FILE * output)
{
    ...
    if (type->attrUses != NULL)
	xmlSchemaAttrUsesDump((struct _xmlSchemaItemList *)type->attrUses, output);
    if (type->annot != NULL)
...
#define WXS_FIND_GLOBAL_ITEM(slot)			\
	...
	ret = (struct _xmlSchemaElement *)xmlHashLookup(schema->slot, name); \
	if (ret != NULL) goto exit; \
    } \
    if (xmlHashSize(schema->schemasImports) > 1) { \
	xmlSchemaImportPtr import; \
	if (nsName == NULL) \
	    import = (struct _xmlSchemaImport *)xmlHashLookup(schema->schemasImports, \
		XML_SCHEMAS_NO_NAMESPACE); \
	else \
	    import = (struct _xmlSchemaImport *)xmlHashLookup(schema->schemasImports, nsName); \
	if (import == NULL) \
	    goto exit; \
	ret = (struct _xmlSchemaElement *)xmlHashLookup(import->schema->slot, name); \
    }
...
//    xmlSchemaTypePtr ret = NULL;
	...
	ret = (xmlSchemaElementPtr)xmlSchemaGetPredefinedType(name, nsName);
	if (ret != NULL)
	    goto exit;
	/*
...
#endif
    return (xmlSchemaAttributePtr)(ret);
}
...
#endif
    return (xmlSchemaAttributeGroupPtr)(ret);
}
...
#endif
    return (xmlSchemaModelGroupDefPtr)(ret);
}
...
exit:
    return (xmlSchemaNotationPtr)(ret);
}
			      ...
			      const xmlChar *nsName)
{
    ...
    */
	...
	ret = (struct _xmlSchemaBasicItem *)bucket->globals->items[i];
	if (ret->type == type) {
	    switch (type) {
		case XML_SCHEMA_TYPE_COMPLEX:
		case XML_SCHEMA_TYPE_SIMPLE:
		  ...
		  const xmlChar *refTargetNs)
{
    ...
    ret->item = (struct _xmlSchemaBasicItem *)item;
...
{
    xmlNodePtr variavel_delete, cur;
     ...
     */
    variavel_delete = NULL;
    cur = root;
    while (cur != NULL) {
        if (variavel_delete != NULL) {
            xmlUnlinkNode(variavel_delete);
            xmlFreeNode(variavel_delete);
            variavel_delete = NULL;
        }
				       ...
				       xmlSchemaWildcardPtr baseWild)
{
    ...
    if (uses != NULL) {
	for (i = 0; i < uses->nbItems; i++) {
	    cur = (struct _xmlSchemaAttributeUse *)uses->items[i];
	    found = 0;
	    if (baseUses == NULL)
		goto not_found;
	    for (j = 0; j < baseUses->nbItems; j++) {
		bcur = (struct _xmlSchemaAttributeUse *)baseUses->items[j];
				  ...
				  xmlSchemaTypePtr type)
{
    ...
    uses = (struct _xmlSchemaItemList *)type->attrUses;
    baseUses = (struct _xmlSchemaItemList *)baseType->attrUses;
    /*
    ...
    */
	...
	if (WXS_IS_RESTRICTION(type)) {
		...
		use = (struct _xmlSchemaAttributeUse *)baseUses->items[i];
		if (prohibs) {
		    /*
		    * Filter out prohibited uses.
		    */
		    for (j = 0; j < prohibs->nbItems; j++) {
			pro = (struct _xmlSchemaAttributeUseProhib *)prohibs->items[j];
		...
		if (usesCount) {
		    ...
		    */
			...
			tmp = (struct _xmlSchemaAttributeUse *)uses->items[j];
				      ...
				      xmlSchemaTypePtr type)
{
    ...
    */
	...
	(struct _xmlSchemaItemList *)type->attrUses, (struct _xmlSchemaItemList *)base->attrUses,
	type->attributeWildcard,
				     ...
				     xmlSchemaItemListPtr list)
{
    ...
    */
    for (i = 0; i < list->nbItems; i++) {
	ref = (struct _xmlSchemaQNameRef *)list->items[i];
				  ...
				  xmlSchemaItemListPtr prohibs)
{
    ...
    for (i = 0; i < list->nbItems; i++) {
	use = (struct _xmlSchemaAttributeUse *)list->items[i];
    ...
    */
	...
	for (i = prohibs->nbItems -1; i >= 0; i--) {
	    prohib = (struct _xmlSchemaAttributeUseProhib *)prohibs->items[i];
	    for (j = 0; j < list->nbItems; j++) {
		use = (struct _xmlSchemaAttributeUse *)list->items[j];
				  ...
				  xmlSchemaAttributeGroupPtr attrGr)
{
	...
	&(attrGr->attributeWildcard), (struct _xmlSchemaItemList *)attrGr->attrUses, (struct _xmlSchemaItemList *)NULL) == -1)
	return(-1);
    return(0);
}
	...
	switch (item->type) {
		...
		*/
		err = xmlSchemaCheckDerivationOKRestriction2to4(pctxt,
		    ...
		    item, redef->target,
		    (struct _xmlSchemaItemList *)(WXS_ATTR_GROUP_CAST item)->attrUses,
		    (struct _xmlSchemaItemList *)(WXS_ATTR_GROUP_CAST redef->target)->attrUses,
...
#define WXS_GET_GLOBAL_HASH(c, slot) { \
    ...
    */
    ...
    for (i = 0; i < bucket->globals->nbItems; i++) {
	item = (struct _xmlSchemaBasicItem *)bucket->globals->items[i];
	table = NULL;
	switch (item->type) {
	    case XML_SCHEMA_TYPE_COMPLEX:
	    case XML_SCHEMA_TYPE_SIMPLE:
...
#define FIXHFAILURE if (pctxt->err == XML_SCHEMAP_INTERNAL) goto exit_failure;
    ...
    */
    for (i = 0; i < nbItems; i++) {
	item = (struct _xmlSchemaTreeItem *)con->pending->items[i];
	switch (item->type) {
	    case XML_SCHEMA_TYPE_COMPLEX:
		if (WXS_IS_TYPE_NOT_FIXED(WXS_TYPE_CAST item)) {
		       ...
		       const xmlChar* nsname)
{
	...
	if ((vctxt->nodeQNames->items [i] == (void*)lname) &&
	    (vctxt->nodeQNames->items[i +1] == (void*)nsname))
	    /* Already there */
	    return(i);
    }
    /* Add new entry. */
    ...
    */
	    ...
	    matcher->targets->nbItems)
	{
	    ...
	    */
	    for (i = 0; i < matcher->targets->nbItems; i++) {
		res = 0;
		refNode = (struct _xmlSchemaPSVIIDCNode *)matcher->targets->items[i];
		if (bind != NULL) {
...
xmlSchemaVAttributesComplex(xmlSchemaValidCtxtPtr vctxt)
{
    ...
    */
    ...
    for (i = 0; i < nbUses; i++) {
        found = 0;
	attrUse = (struct _xmlSchemaAttributeUse *)attrUseList->items[i];
			      ...
			      xmlSchemaValPtr *val)
{
	...
	*/
	if ((! WXS_HAS_SIMPLE_CONTENT(inode->typeDef)) &&
	    ...
	    /* NOTE that this covers (2.2.2) as well. */
	    VERROR((xmlParserErrors)ret, NULL,
		"For a string to be a valid default, the type definition "
	...
	*/
	...
	case XML_SCHEMA_CONTENT_BASIC:
		...
		*/
		...
		VERROR((xmlParserErrors)ret, NULL, "Element content is not allowed, "
...
xmlSchemaVDocWalk(xmlSchemaValidCtxtPtr vctxt)
{
    ...
    if (valRoot == NULL) {
	/* VAL TODO: Error code? */
	VERROR((xmlParserErrors)1, NULL, "The document has no document element");
	return (1);
    }
    vctxt->depth = -1;
