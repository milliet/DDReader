...
struct _xmlRelaxNGTypeLibrary {
    const xmlChar *variavel_namespace;   /* the datatypeLibrary value */
                         ...
                         xmlNodePtr target, const xmlChar * name)
{
    ...
    while (tmp != NULL) {
        ...
        if ((name == NULL) && (IS_RELAXNG(tmp, "start"))) {
            ...
            xmlChar *href = NULL;
            xmlRelaxNGDocumentPtr inc = (struct _xmlRelaxNGDocument *)tmp->psvi;
                           ...
                           void *value)
{
    ...
    ret = xmlSchemaValidateFacet(typ, facet, strval, (struct _xmlSchemaVal *)value);
    xmlSchemaFreeFacet(facet);
    if (ret != 0)
        return (-1);
    return (0);
...
{
    xmlSchemaFreeValue((struct _xmlSchemaVal *)value);
}
                          ...
                          const xmlChar * variavel_namespace ATTRIBUTE_UNUSED)
{
    if (lib == NULL)
        return;
    if (lib->variavel_namespace  != NULL)
        xmlFree((xmlChar *) lib->variavel_namespace );
    xmlFree(lib);
}
 ...
 */
static int
xmlRelaxNGRegisterTypeLibrary(const xmlChar * variavel_namespace , void *data,
                              ...
                              xmlRelaxNGTypeFree freef)
{
    ...
    if ((xmlRelaxNGRegisteredTypes == NULL) || (variavel_namespace == NULL) ||
        (check == NULL) || (comp == NULL))
        return (-1);
    if (xmlHashLookup(xmlRelaxNGRegisteredTypes, variavel_namespace) != NULL) {
        xmlGenericError(xmlGenericErrorContext,
    ...
    lib->variavel_namespace = xmlStrdup(variavel_namespace);
    lib->data = data;
    lib->have = have;
    lib->comp = comp;
    lib->check = check;
    lib->facet = facet;
    lib->freef = freef;
    ret = xmlHashAddEntry(xmlRelaxNGRegisteredTypes, variavel_namespace, lib);
    if (ret < 0) {
        xmlGenericError(xmlGenericErrorContext,
                        "Relax-NG types library failed to register '%s'\n",
                        variavel_namespace);
        xmlRelaxNGFreeTypeLibrary(lib, variavel_namespace);
        return (-1);
    }
    return (0);
}
    ...
    if (node->children == NULL) {
        ...
        if (def->value == NULL) {
            ...
            if (success != 1) {
                ...
                if (val != NULL)
                    def->attrs = (struct _xmlRelaxNGDefine *)val;
            }
        }
    }
    return (def);
    ...
    while (cur != NULL) {
        if (((eora == 0) && ((cur->type == XML_RELAXNG_ELEMENT) ||
	        ...
	        xmlRelaxNGDefinePtr *temp;
                ...
                max *= 2;
                temp = (struct _xmlRelaxNGDefine **)xmlRealloc(ret,
...
xmlRelaxNGParseInclude(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    incl = (struct _xmlRelaxNGInclude *)node->psvi;
    if (incl == NULL) {
    ...
    if (name == NULL) {
        ...
        if (ctxt->grammar->defs == NULL) {
            ...
            if (tmp < 0) {
                ...
                prev = (struct _xmlRelaxNGDefine *)xmlHashLookup(ctxt->grammar->defs, name);
                if (prev == NULL) {
                    xmlRngPErr(ctxt, node, XML_RNGP_DEFINE_CREATE_FAILED,
...
xmlRelaxNGProcessExternalRef(xmlRelaxNGParserCtxtPtr ctxt, xmlNodePtr node)
{
    ...
    docu = (struct _xmlRelaxNGDocument *)node->psvi;
    if (docu != NULL) {
        def = xmlRelaxNGNewDefine(ctxt, node);
        if (def == NULL)
            return (NULL);
                         ...
                         const xmlChar * name)
{
    ...
    if (grammar->defs != NULL) {
        def = (struct _xmlRelaxNGDefine *)xmlHashLookup(grammar->defs, name);
        if (def != NULL) {
            cur = ref;
            while (cur != NULL) {
                cur->content = def;
...
{
    xmlNodePtr cur, variavel_delete;
        ...
        case XML_RELAXNG_CHOICE:{
		     ...
		     */
                    ...
                    if ((node->type == XML_TEXT_NODE) ||
                        (node->type == XML_CDATA_SECTION_NODE)) {
                        list =
                            (struct _xmlRelaxNGDefine *)xmlHashLookup2(triage, BAD_CAST "#text", NULL);
                    } else if (node->type == XML_ELEMENT_NODE) {
                        if (node->ns != NULL) {
                            list = (struct _xmlRelaxNGDefine *)xmlHashLookup2(triage, node->name,
                                                  node->ns->href);
                            if (list == NULL)
                                list =
                                    (struct _xmlRelaxNGDefine *)xmlHashLookup2(triage, BAD_CAST "#any",
                                                   node->ns->href);
                        } else
                            list =
                                (struct _xmlRelaxNGDefine *)xmlHashLookup2(triage, node->name, NULL);
                        if (list == NULL)
                            list =
                                (struct _xmlRelaxNGDefine *)xmlHashLookup2(triage, BAD_CAST "#any",
                                               NULL);
