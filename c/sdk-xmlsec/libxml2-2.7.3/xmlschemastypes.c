			     ...
			     unsigned long *length)  
{
	...
	(xmlSchemaTypeType)type->builtInType, value, val, length,
...
xmlSchemaValidateLengthFacetWhtsp(xmlSchemaFacetPtr facet,
				  ...
				  xmlSchemaWhitespaceValueType ws)
{
    return (xmlSchemaValidateLengthFacetInternal(facet, (xmlSchemaTypeType)valType, value, val,
	length, ws));
}
    ...
    *retValue = NULL;
	...
	case XML_SCHEMAS_UINT:
		...
		*retValue = xmlStrdup(BAD_CAST "0");
		...
		if (dec.sign)
		    bufsize++;
		*retValue = (const unsigned char *)xmlMalloc(bufsize);
		if (*retValue == NULL)
		    return(-1);
		if (dec.hi != 0) {
		    if (dec.sign)
	...
	case XML_SCHEMAS_GMONTH: {
		/* TODO: Unclear in XML Schema 1.0 */
		/* TODO: What to do with the timezone? */
		*retValue = (const unsigned char *)xmlMalloc(6);
		if (*retValue == NULL)
		    return(-1);
		...
		*retValue = (const unsigned char *)xmlMalloc(6);
		...
		/* TODO: Unclear in XML Schema 1.0 */
		/* TODO: What to do with the timezone? */
		*retValue = (const unsigned char *)xmlMalloc(8);
		if (*retValue == NULL)
		    return(-1);
