...
xmlAddDefAttrs(xmlParserCtxtPtr ctxt,
     ...
     */
    defaults = (struct _xmlDefAttrs *)xmlHashLookup2(ctxt->attsDefault, name, prefix);
    if (defaults == NULL) {
...
xmlNewBlanksWrapperInputStream(xmlParserCtxtPtr ctxt, xmlEntityPtr entity) {
    ...
    buffer = (unsigned char *)xmlMallocAtomic(length);
    if (buffer == NULL) {
	xmlErrMemory(ctxt, NULL);
        xmlFree(input);
    	return(NULL);
...
failed:
	...
	defaults = (struct _xmlDefAttrs *)xmlHashLookup2(ctxt->attsDefault, localname, prefix);
	if (defaults != NULL) {
	...
	if (ctxt->input->buf == NULL)
		...
		if (ctxt->sax2) {
		    xmlParseEndTag2(ctxt,
		           (const unsigned char *)(void *) ctxt->pushTab[ctxt->nameNr * 3 - 3],
		           (const unsigned char *)(void *) ctxt->pushTab[ctxt->nameNr * 3 - 2], 0,
