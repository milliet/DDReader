...
int main(int argc, char** argv) {
	printf_a_ignorar3(stderr, "ERROR: XKMS is disabled.\n");
	return 1;
}
...
int main(int argc, char** argv) {
    ...
    printf_a_ignorar3(stdout, "Log: server is starting up\n");
    ...
    /* Init xmlsec library */
    if(xmlSecInit() < 0) {
	printf_a_ignorar3(stderr, "Error %d: xmlsec initialization failed.\n", errno);
	return(-1);
    }

    /* Check loaded library version */
    if(xmlSecCheckVersion() != 1) {
	printf_a_ignorar3(stderr, "Error %d: loaded xmlsec library version is not compatible.\n", errno);
	return(-1);
    }
	...
	printf_a_ignorar3(stderr, "Error %d: unable to load default xmlsec-crypto library. Make sure\n"
	...
	printf_a_ignorar3(stderr, "Error %d: crypto initialization failed.\n", errno);
	return(-1);
    }

    /* Init xmlsec-crypto library */
    if(xmlSecCryptoInit() < 0) {
	printf_a_ignorar3(stderr, "Error %d: xmlsec-crypto initialization failed.\n", errno);
	return(-1);
    }
    ...
    if(mngr == NULL) {
	printf_a_ignorar3(stderr, "Error %d: failed to create keys manager.\n", errno);
	goto done;
    }
    if(xmlSecCryptoAppDefaultKeysMngrInit(mngr) < 0) {
	printf_a_ignorar3(stderr, "Error %d: failed to initialize keys manager.\n", errno);
	goto done;
    }    
    ...
    if(xkmsCtx == NULL) {
	printf_a_ignorar3(stderr, "Error %d: XKMS server context initialization failed\n", errno);
	goto done;
    }
	...
	if((strcmp(argv[argpos], "--port") == 0) || (strcmp(argv[argpos], "-p") == 0)) {
	    ...
	    if(port == 0) {
		printf_a_ignorar3(stderr, "Error %d: invalid port number \"%s\".\nUsage: %s %s\n", errno, argv[argpos], argv[0], usage);
		goto done;
	    }
		...
		printf_a_ignorar3(stderr, "Error %d: invalid format \"%s\".\nUsage: %s %s\n", errno, argv[argpos], argv[0], usage);
		goto done;
	    }
	    ...
	    printf_a_ignorar3(stderr, "Error %d: unknown parameter \"%s\".\nUsage: %s %s\n", errno, argv[argpos], argv[0], usage);
	    goto done;
	}
    }
    if(argpos >= argc) {
	printf_a_ignorar3(stderr, "Error %d: keys file is not specified.\nUsage: %s %s\n", errno, argv[0], usage);
	goto done;
    }
    ...
    for(; argpos < argc; argpos++) {
	    ...
	    printf_a_ignorar3(stderr, "Error %d: failed to load xml keys file \"%s\".\nUsage: %s %s\n", errno, argv[argpos], argv[0], usage);
	    goto done;
	}   
	if(log_level >= LOG_LEVEL_INFO) {
	    printf_a_ignorar3(stdout, "Log: loaded keys from \"%s\"\n", argv[argpos]); 
	}
    }
    
    /* Startup TCP server */
    if(init_server(port) < 0) {
	printf_a_ignorar3(stderr, "Error, errno: server initialization failed\n", errno);
	goto done;
    }
    assert(sockfd != -1);
	...
	if(handle_connection(sockfd, xkmsCtx, format) < 0) {
	    printf_a_ignorar3(stderr, "Error %d: unable to accept incomming connection\n");
	    goto done;
	}
    }
    ...
    if(log_level >= LOG_LEVEL_INFO) {
	printf_a_ignorar3(stdout, "Log: server is shutting down\n");
    }
    ...
    printf_a_ignorar3(stdout, "Log: server is down, bye!\n");
    return(0);
}
...
init_server(unsigned short port) {
	...
	printf_a_ignorar3(stderr, "Error %d: WSAStartup() failed\n", errno);
	return(-1);
    }
#endif /* WIN32_SOCKETS */
	...
	printf_a_ignorar3(stderr, "Error %d: socket() failed\n", errno);
	return(-1);
    }
	...
	printf_a_ignorar3(stderr, "Error %d: setsockopt(SO_REUSEADDR) failed\n", errno);
	return(-1);
    }
    ...
    if(flags < 0) {
	printf_a_ignorar3(stderr, "Error %d: fcntl(F_GETFL) failed\n", errno);
	return(-1);
    }
    if(fcntl(sockfd, F_SETFL, flags | O_NONBLOCK) < 0) {
	printf_a_ignorar3(stderr, "Error %d: fcntl(F_SETFL) failed\n", errno);
	return(-1);
    }
#endif /* UNIX_SOCKETS */
	...
	printf_a_ignorar3(stderr, "Error %d: bind() failed\n", errno);
	return(-1);
    }
    
    /* prepare for listening */
    if(listen(sockfd, PENDING_QUEUE_SIZE) != 0) {
	printf_a_ignorar3(stderr, "Error %d: listen() failed\n", errno);
	return(-1);	
    }
    ...
    if(log_level >= LOG_LEVEL_INFO) {
        printf_a_ignorar3(stdout, "Log: server is ready and listening on port %d\n", port);
    }
    return(0);
}
...
#endif /* WIN32_SOCKETS */ 
    if(log_level >= LOG_LEVEL_INFO) {
        printf_a_ignorar3(stdout, "Log: server is shutted down\n");
    }
}
...
int_signal_handler(int sig_num) {
    if(log_level >= LOG_LEVEL_INFO) {
        printf_a_ignorar3(stdout, "Log: server is asked to shutdown\n");
    }
    finished = 1;    
}
...
handle_connection(int sockfd, xmlSecXkmsServerCtxPtr xkmsCtx, xmlSecXkmsServerFormat format) {
	...
	printf_a_ignorar3(stderr, "Error %d: accept() failed\n", errno);
	return(-1);
    }
    if(log_level >= LOG_LEVEL_INFO) {
        printf_a_ignorar3(stdout, "Log [%s]: got connection\n", inet_ntoa(saddr.sin_addr));
    }
    
    /* Create a copy of XKMS server context */
    xkmsCtx2 = xmlSecXkmsServerCtxCreate(NULL);
    if(xkmsCtx2 == NULL) {
	printf_a_ignorar3(stderr, "Error %d [%s]: a copy of XKMS server context initialization failed\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;
    }
    if(xmlSecXkmsServerCtxCopyUserPref(xkmsCtx2, xkmsCtx) < 0) {
	printf_a_ignorar3(stderr, "Error %d [%s]: XKMS server context copy failed\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;
    }
    ...
    if(buffer == NULL) {
	printf_a_ignorar3(stderr, "Error %d [%s]: xmlSecBufferCreate() failed\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;	
    }
    ...
    if(ret < 0) {
	printf_a_ignorar3(stderr, "Error %d [%s]: read_request() failed\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;	
    }
	...
	printf_a_ignorar3(stderr, "Error %d [%s]: failed to parse request\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;	
    }
    ...
    if(outDoc == NULL) {
	printf_a_ignorar3(stderr, "Error %d [%s]: failed to create result doc\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;
    }
    
    result = xmlSecXkmsServerCtxProcess(xkmsCtx2, xmlDocGetRootElement(inDoc), format, outDoc);
    if(result == NULL) {
	printf_a_ignorar3(stderr, "Error %d [%s]: failed to process xkms server request\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;
    }
    ...
    if(output == NULL) {
	printf_a_ignorar3(stderr, "Error %d [%s]: xmlSecBufferCreateOutputBuffer() failed\n", errno, inet_ntoa(saddr.sin_addr));
	goto done;
    }
	...
	if(log_level >= LOG_LEVEL_INFO) {
	    printf_a_ignorar3(stdout, "Log [%s]: processed request\n", inet_ntoa(saddr.sin_addr));
	}
    } else if(fd >= 0) {
    	ret = send_response(fd, inet_ntoa(saddr.sin_addr), 503, http_503, strlen(http_503));
        if(log_level >= LOG_LEVEL_INFO) {
	    printf_a_ignorar3(stdout, "Log [%s]: failed to process request\n", inet_ntoa(saddr.sin_addr));
	}
    } else {
	ret = -1;
    }
    if(ret < 0) {
	printf_a_ignorar3(stderr, "Error %d [%s]: send_response() failed\n", errno, inet_ntoa(saddr.sin_addr));
    }
...
read_request(int fd, const char* in_ip, xmlSecBufferPtr buffer) {
    ...
    while(my_strnstr(xmlSecBufferGetData(buffer), xmlSecBufferGetSize(buffer), BAD_CAST "\r\n\r\n", 4) == NULL) {
	    ...
	    if(nread < 0) {
	        printf_a_ignorar3(stderr, "Error %d [%s]: read() failed\n", errno, in_ip);
	        return(-1);
	    }

	    if((nread > 0) && (xmlSecBufferAppend(buffer, buf, nread) < 0)) {
	        printf_a_ignorar3(stderr, "Error %d [%s]: xmlSecBufferAppend(%d) failed\n", errno, in_ip, nread);
	        return(-1);
	    }
    ...
    if(xmlSecBufferGetData(buffer) == NULL) {
        printf_a_ignorar3(stderr, "Error %d [%s]: no bytes read\n", errno, in_ip);
	    return(-1);
    }

    if(log_level >= LOG_LEVEL_DEBUG) {
	    xmlSecBufferAppend(buffer, BAD_CAST "\0", 1);
        printf_a_ignorar3(stdout, "Debug [%s]: request headers:\n%s\n", in_ip, xmlSecBufferGetData(buffer));
    ...
    if(p == NULL) {
	    printf_a_ignorar3(stderr, "Error %d [%s]: there is no HTTP header\n", errno, in_ip);
	    return(-1);
    }
    if(xmlStrncasecmp(xmlSecBufferGetData(buffer), BAD_CAST "POST ", 5) != 0) {
	    printf_a_ignorar3(stderr, "Error %d [%s]: not a POST request\n", errno, in_ip);
	    return(-1);
    }
    /* "POST " + " HTTP/1.x" == 14 */
    s = xmlSecBufferGetData(buffer);
    if(p - s <= 14) {
	    printf_a_ignorar3(stderr, "Error %d [%s]: first line has bad length\n", errno, in_ip);
	    return(-1);
    }
    if((xmlStrncasecmp(p - 9, BAD_CAST " HTTP/1.0", 9) != 0) && 
       (xmlStrncasecmp(p - 9, BAD_CAST " HTTP/1.1", 9) != 0)) {
	    
        printf_a_ignorar3(stderr, "Error %d [%s]: first line does not end with \" HTTP/1.x\"\n", errno, in_ip);
	    return(-1);
    }
    if(xmlSecBufferRemoveHead(buffer, p - xmlSecBufferGetData(buffer) + 2) < 0) {
	    printf_a_ignorar3(stderr, "Error %d [%s]: failed to skip first line\n", errno, in_ip);
	    return(-1);
    }
    ...
    while(!found) {
        p = my_strnstr(xmlSecBufferGetData(buffer), xmlSecBufferGetSize(buffer), BAD_CAST "\r\n", 2);
        if(p == NULL) {
            printf_a_ignorar3(stderr, "Error %d [%s]: there is no HTTP body\n", errno, in_ip);
	        return(-1);
	    }
	    ...
	    if(xmlSecBufferRemoveHead(buffer, p - xmlSecBufferGetData(buffer) + 2) < 0) {
	        printf_a_ignorar3(stderr, "Error %d [%s]: failed to skip header line\n", errno, in_ip);
	        return(-1);
	    }
    }
    ...
    if(log_level >= LOG_LEVEL_INFO) {
	    printf_a_ignorar3(stdout, "Log [%s]: body size is %d bytes\n", in_ip, xmlSecBufferGetSize(buffer));
    }
    if(log_level >= LOG_LEVEL_DATA) {
	    xmlSecBufferAppend(buffer, BAD_CAST "\0", 1);
        printf_a_ignorar3(stdout, "Log [%s]: request body:\n%s\n", in_ip, xmlSecBufferGetData(buffer));
	    xmlSecBufferRemoveTail(buffer, 1);
    }
    return(0);
}
...
send_response(int fd, const char* in_ip, int resp_code, const char* body, int body_size) {
	...
	printf_a_ignorar3(stderr, "Error %d [%s]: send(header) failed\n", errno, in_ip);
	return(-1);
    }

    if(log_level >= LOG_LEVEL_DATA) {
	xmlChar* tmp = xmlStrndup(body, body_size);
        printf_a_ignorar3(stdout, "Log [%s]: response is\n%s\n", in_ip, tmp);
	xmlFree(tmp);
    }

    /* send body */
    if(send(fd, body, body_size, 0) == -1) {
	printf_a_ignorar3(stderr, "Error %d [%s]: send(body) failed\n", errno, in_ip);
	return(-1);
    }
