...
main(int argc, char **argv) {
	...
	printf_a_ignorar3(stderr, "Usage: %s <xml-file> <key-file> <cert-file>\n", argv[0]);
	return(1);
    }
...
sign_file(const char* xml_file, const char* key_file, const char* cert_file) {
	...
	printf_a_ignorar3(stderr, "Error: failed to add X509Data node\n");
	goto done;		
    }
    ...
    if(xmlSecCryptoAppKeyCertLoad(dsigCtx->signKey, cert_file, xmlSecKeyDataFormatPem) < 0) {
        printf_a_ignorar3(stderr,"Error: failed to load pem certificate \"%s\"\n", cert_file);
	goto done;
    }
