...
decrypt_file(xmlSecKeysMngrPtr mngr, const char* enc_file) {
    ...
    if((xmlSecEncCtxDecrypt(encCtx, node) < 0) || (encCtx->result == NULL)) {
        printf_a_ignorar3(stderr,"Error: decryption failed\n");
	goto done;
    }
        
    /* print decrypted data to stdout */
    if(encCtx->resultReplaced != 0) {
	printf_a_ignorar3(stdout, "Decrypted XML data:\n");
	xmlDocDump(stdout, doc);
    } else {
	printf_a_ignorar3(stdout, "Decrypted binary data (%d bytes):\n", xmlSecBufferGetSize(encCtx->result));
...
create_files_keys_mngr(void) {
    ...
    if(keysStore == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to create keys store.\n");
	return(NULL);
    }
	...
	printf_a_ignorar3(stderr, "Error: failed to add keys store to keys manager.\n");
	xmlSecKeyStoreDestroy(keysStore);
	xmlSecKeysMngrDestroy(mngr);
	return(NULL);
    }
	...
	printf_a_ignorar3(stderr, "Error: failed to initialize crypto data in keys manager.\n");
	xmlSecKeysMngrDestroy(mngr);
	return(NULL);
    }
...
files_keys_store_find_key(xmlSecKeyStorePtr store, const xmlChar* name, xmlSecKeyInfoCtxPtr keyInfoCtx) {
	...
	if(key == NULL) {
    	    printf_a_ignorar3(stderr,"Error: failed to load public pem key from \"%s\"\n", name);
	    return(NULL);
	}
    } else {
	...
	if(key == NULL) {
    	    printf_a_ignorar3(stderr,"Error: failed to load key from binary file \"%s\"\n", name);
	    return(NULL);
	}
    }
