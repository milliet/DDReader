...
main(int argc, char **argv) {
    ...
    /* start response */
    printf_a_ignorar3(stdout, "Content-type: text/plain\n");
    ...
    /* Init xmlsec library */
    if(xmlSecInit() < 0) {
	printf_a_ignorar3(stdout, "Error: xmlsec initialization failed.\n");
	return(-1);
    }

    /* Check loaded library version */
    if(xmlSecCheckVersion() != 1) {
	printf_a_ignorar3(stdout, "Error: loaded xmlsec library version is not compatible.\n");
	return(-1);
    }
	...
	printf_a_ignorar3(stdout, "Error: unable to load default xmlsec-crypto library. Make sure\n"
	...
	printf_a_ignorar3(stdout, "Error: crypto initialization failed.\n");
	return(-1);
    }

    /* Init xmlsec-crypto library */
    if(xmlSecCryptoInit() < 0) {
	printf_a_ignorar3(stdout, "Error: xmlsec-crypto initialization failed.\n");
	return(-1);
    }
    ...
    if(mngr == NULL) {
	printf_a_ignorar3(stdout, "Error: failed to create keys manager.\n");
	return(-1);
    }
    if(xmlSecCryptoAppDefaultKeysMngrInit(mngr) < 0) {
	printf_a_ignorar3(stdout, "Error: failed to initialize keys manager.\n");
	return(-1);
    }    
...
int load_trusted_certs(xmlSecKeysMngrPtr mngr, const char* path, int report_loaded_certs) {
    ...
    dir = opendir(path);
    if(dir == NULL) {
	printf_a_ignorar3(stdout, "Error: failed to open folder \"%s\".\n", path);
	return(-1);
    }    
	...
	if((len > 4) && (strcmp(entry->d_name + len - 4, ".pem") == 0)) {
	    ...
	    if(xmlSecCryptoAppKeysMngrCertLoad(mngr, filename, xmlSecKeyDataFormatPem, xmlSecKeyDataTypeTrusted) < 0) {
    		printf_a_ignorar3(stdout,"Error: failed to load pem certificate from \"%s\"\n", filename);
		closedir(dir);
		return(-1);
	    }
	    if(report_loaded_certs) {		    	    
    		printf_a_ignorar3(stdout, "Loaded trusted certificate from \"%s\"...\n", filename);
	    }
	    ...
	    if(xmlSecCryptoAppKeysMngrCertLoad(mngr, filename, xmlSecKeyDataFormatDer, xmlSecKeyDataTypeTrusted) < 0) {
    		printf_a_ignorar3(stdout,"Error: failed to load der certificate from \"%s\"\n", filename);
		closedir(dir);
		return(-1);
	    }
	    if(report_loaded_certs) {		    	    
...
int load_keys(xmlSecKeysMngrPtr mngr, const char* path, int report_loaded_keys) {
    ...
    if(xmlSecCryptoAppDefaultKeysMngrLoad(mngr, filename) < 0) {
    	printf_a_ignorar3(stdout,"Error: failed to load keys from \"%s\"\n", filename);
	return(-1);
    }
    
    if(report_loaded_keys) {
        printf_a_ignorar3(stdout, "Loaded keys from \"%s\"...\n", filename);
    }
    return(0);
}
...
verify_request(xmlSecKeysMngrPtr mngr) {
    ...
    if(buffer == NULL) {
    	printf_a_ignorar3(stdout,"Error: failed to create buffer\n");
	goto done; 	
    }
	...
	if(ret < 0) {
	    printf_a_ignorar3(stdout,"Error: read failed\n");
	    goto done; 	
	}
    ...
    if (doc == NULL) {
	printf_a_ignorar3(stdout, "Error: unable to parse xml document (syntax error)\n");
	goto done;
    }
    ...
    if(xmlDocGetRootElement(doc) == NULL) {
        printf_a_ignorar3(stdout,"Error: empty document\n");
	goto done;
    }
    ...
    if(node == NULL) {
	printf_a_ignorar3(stdout, "Error: start <dsig:Signature/> node not found\n");
	goto done;	
    }
    ...
    if(dsigCtx == NULL) {
        printf_a_ignorar3(stdout,"Error: failed to create signature context\n");
	goto done;
    }
    ...
    */
    ...
    if(xmlSecDSigCtxVerify(dsigCtx, node) < 0) {
        printf_a_ignorar3(stdout,"Error: signature verification failed\n");
	goto done;
    }
        
    /* print verification result to stdout */
    if(dsigCtx->status == xmlSecDSigStatusSucceeded) {
	printf_a_ignorar3(stdout, "RESULT: Signature is OK\n");
    } else {
	printf_a_ignorar3(stdout, "RESULT: Signature is INVALID\n");
    }    
    printf_a_ignorar3(stdout, "---------------------------------------------------\n");
