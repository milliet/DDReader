...
xmlSecNodeSetDebugDump(xmlSecNodeSetPtr nset, FILE *output) {
    ...
    printf_a_ignorar2(output, "== Nodes set ");
    switch(nset->type) {
    case xmlSecNodeSetNormal:
	printf_a_ignorar2(output, "(xmlSecNodeSetNormal)\n");
	break;
    case xmlSecNodeSetInvert:
	printf_a_ignorar2(output, "(xmlSecNodeSetInvert)\n");
	break;
    case xmlSecNodeSetTree:
	printf_a_ignorar2(output, "(xmlSecNodeSetTree)\n");
	break;
    case xmlSecNodeSetTreeWithoutComments:
	printf_a_ignorar2(output, "(xmlSecNodeSetTreeWithoutComments)\n");
	break;
    case xmlSecNodeSetTreeInvert:
	printf_a_ignorar2(output, "(xmlSecNodeSetTreeInvert)\n");
	break;
    case xmlSecNodeSetTreeWithoutCommentsInvert:
	printf_a_ignorar2(output, "(xmlSecNodeSetTreeWithoutCommentsInvert)\n");
	break;
    case xmlSecNodeSetList:
	printf_a_ignorar2(output, "(xmlSecNodeSetList)\n");
	printf_a_ignorar2(output, ">>>\n");
	xmlSecNodeSetDebugDump(nset->children, output);
	printf_a_ignorar2(output, "<<<\n");
	return;
    default:
	printf_a_ignorar3(output, "(unknown=%d)\n", nset->type);
	xmlSecErr_a_ignorar6(XMLSEC_ERRORS_HERE,
		    NULL,
		    NULL,
		    XMLSEC_ERRORS_R_INVALID_TYPE,
	...
	if(cur->type != XML_NAMESPACE_DECL) {
	    printf_a_ignorar4(output, "%d: %s\n", cur->type, 
		(cur->name) ? cur->name : BAD_CAST "null");
	} else {
	    xmlNsPtr ns = (xmlNsPtr)cur;
		/*
	    printf_a_ignorar3(output, "%d: %s=%s (%s:%s)\n", cur->type, 
