...
xmlSecKeyUseWithDebugDump(xmlSecKeyUseWithPtr keyUseWith, FILE* output) {
    ...
    printf_a_ignorar4(output, "=== KeyUseWith: application=\"%s\",identifier=\"%s\"\n", 
...
xmlSecKeyUseWithDebugXmlDump(xmlSecKeyUseWithPtr keyUseWith, FILE* output) {
    ...
    printf_a_ignorar2(output, "<KeyUseWith>\n");
    printf_a_ignorar3(output, "<Application>%s</Application>", 
        (keyUseWith->application) ? keyUseWith->application : BAD_CAST "");
    printf_a_ignorar3(output, "<Identifier>%s</Identifier>", 
        (keyUseWith->identifier) ? keyUseWith->identifier : BAD_CAST "");
    printf_a_ignorar2(output, "</KeyUseWith>\n");
}
...
xmlSecKeyReqDebugDump(xmlSecKeyReqPtr keyReq, FILE* output) {
    ...
    printf_a_ignorar2(output, "=== KeyReq:\n");
    printf_a_ignorar3(output, "==== keyId: %s\n", 
	    (xmlSecKeyDataKlassGetName(keyReq->keyId)) ? 
		xmlSecKeyDataKlassGetName(keyReq->keyId) : 
		BAD_CAST "NULL");
    printf_a_ignorar3(output, "==== keyType: 0x%08x\n", keyReq->keyType);
    printf_a_ignorar3(output, "==== keyUsage: 0x%08x\n", keyReq->keyUsage);
    printf_a_ignorar3(output, "==== keyBitsSize: %d\n", keyReq->keyBitsSize);
...
xmlSecKeyReqDebugXmlDump(xmlSecKeyReqPtr keyReq, FILE* output) {
    ...
    printf_a_ignorar2(output, "<KeyReq>\n");
    printf_a_ignorar3(output, "<KeyId>%s</KeyId>\n", 
	    (xmlSecKeyDataKlassGetName(keyReq->keyId)) ? 
		xmlSecKeyDataKlassGetName(keyReq->keyId) : 
		BAD_CAST "NULL");
    printf_a_ignorar3(output, "<KeyType>0x%08x</KeyType>\n", keyReq->keyType);
    printf_a_ignorar3(output, "<KeyUsage>0x%08x</KeyUsage>\n", keyReq->keyUsage);
    printf_a_ignorar3(output, "<KeyBitsSize>%d</KeyBitsSize>\n", keyReq->keyBitsSize);
...
xmlSecKeyDebugDump(xmlSecKeyPtr key, FILE *output) {
    ...
    printf_a_ignorar2(output, "== KEY\n");
    printf_a_ignorar3(output, "=== method: %s\n", 
	    (key->value->id->dataNodeName != NULL) ? 
	    (char*)(key->value->id->dataNodeName) : "NULL"); 

    printf_a_ignorar2(output, "=== key type: ");
    if((xmlSecKeyGetType(key) & xmlSecKeyDataTypeSymmetric) != 0) {
	printf_a_ignorar2(output, "Symmetric\n");
    } else if((xmlSecKeyGetType(key) & xmlSecKeyDataTypePrivate) != 0) {
	printf_a_ignorar2(output, "Private\n");
    } else if((xmlSecKeyGetType(key) & xmlSecKeyDataTypePublic) != 0) {
	printf_a_ignorar2(output, "Public\n");
    } else {
	printf_a_ignorar2(output, "Unknown\n");
    } 

    if(key->name != NULL) {
	printf_a_ignorar3(output, "=== key name: %s\n", key->name);
    }
    printf_a_ignorar3(output, "=== key usage: %d\n", key->usage);
    if(key->notValidBefore < key->notValidAfter) {
        printf_a_ignorar3(output, "=== key not valid before: %ld\n", (unsigned long)key->notValidBefore);
	printf_a_ignorar3(output, "=== key not valid after: %ld\n", (unsigned long)key->notValidAfter);
    }
    if(key->value != NULL) {
...
xmlSecKeyDebugXmlDump(xmlSecKeyPtr key, FILE *output) {
    ...
    printf_a_ignorar2(output, "<KeyInfo>\n");
    if(key->value->id->dataNodeName != NULL) {
        printf_a_ignorar3(output, "<KeyMethod>%s</KeyMethod>\n", 
		key->value->id->dataNodeName); 
    }

    printf_a_ignorar2(output, "<KeyType>");
    if((xmlSecKeyGetType(key) & xmlSecKeyDataTypeSymmetric) != 0) {
	printf_a_ignorar2(output, "Symmetric\n");
    } else if((xmlSecKeyGetType(key) & xmlSecKeyDataTypePrivate) != 0) {
	printf_a_ignorar2(output, "Private\n");
    ...
    if(key->name != NULL) {
	printf_a_ignorar3(output, "<KeyName>%s</KeyName>\n", key->name);
    }
    if(key->notValidBefore < key->notValidAfter) {
        printf_a_ignorar4(output, "<KeyValidity notValidBefore=\"%ld\" notValidAfter=\"%ld\"/>\n",
		(unsigned long)key->notValidBefore, 
		(unsigned long)key->notValidAfter);
    }
