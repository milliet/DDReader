...
xmlSecXkmsServerCtxWriteSoap11FatalError(xmlSecXkmsServerCtxPtr ctx, xmlNodePtr envNode) {
    ...
    if((ctx->resultMajor == xmlSecXkmsResultMajorVersionMismatch) ||
       (ctx->requestNode == NULL)) {
        ...
        faultString = (unsigned char *)xmlMalloc(len + 1);
        if(faultString == NULL) {
            xmlSecErr_a_ignorar5(XMLSEC_ERRORS_HERE,
		        NULL,
		        "xmlMalloc",
...
xmlSecXkmsServerCtxWriteSoap12FatalError(xmlSecXkmsServerCtxPtr ctx, xmlNodePtr envNode) {
    ...
    if((ctx->resultMajor == xmlSecXkmsResultMajorVersionMismatch) ||
       (ctx->requestNode == NULL)) {
        ...
        len = xmlStrlen(BAD_CAST xmlSecErrorsSafeString(ctx->requestNode->name)) +
              xmlStrlen(xmlSecXkmsSoapFaultReasonMessageInvalid) + 1;
        faultReason = (unsigned char *)xmlMalloc(len + 1);
        if(faultReason == NULL) {
            xmlSecErr_a_ignorar5(XMLSEC_ERRORS_HERE,
		        NULL,
		        "xmlMalloc",
...
xmlSecXkmsServerCtxDebugDump(xmlSecXkmsServerCtxPtr ctx, FILE* output) {
    ...
    printf_a_ignorar3(output, "= XKMS SERVER CONTEXT: %s\n",
	    (ctx->requestId != xmlSecXkmsServerRequestIdUnknown && 
    ...
    printf_a_ignorar3(output, "== id: %s\n", 
		(ctx->id) ? ctx->id : BAD_CAST "");
    printf_a_ignorar3(output, "== service: %s\n", 
		(ctx->service) ? ctx->service : BAD_CAST "");
    printf_a_ignorar3(output, "== nonce: %s\n", 
		(ctx->nonce) ? ctx->nonce : BAD_CAST "");
    printf_a_ignorar3(output, "== originalRequestId: %s\n", 
		(ctx->originalRequestId) ? ctx->originalRequestId : BAD_CAST "");
    printf_a_ignorar3(output, "== pendingNotificationMechanism: %s\n", 
		(ctx->pendingNotificationMechanism) ? 
		    ctx->pendingNotificationMechanism : 
		    BAD_CAST "");
    printf_a_ignorar3(output, "== pendingNotificationIdentifier: %s\n", 
		(ctx->pendingNotificationIdentifier) ? 
		    ctx->pendingNotificationIdentifier : 
		    BAD_CAST "");
    if(ctx->responseLimit != XMLSEC_XKMS_NO_RESPONSE_LIMIT) {
        printf_a_ignorar3(output, "== ResponseLimit: %d\n", ctx->responseLimit);
    }
    xmlSecQName2BitMaskDebugDump(gXmlSecXkmsResponseMechanismInfo, 
		ctx->responseMechanismMask, BAD_CAST "responseMechanism", output);    

    if(ctx->expectedService != NULL) {    
        printf_a_ignorar3(output, "== expected service: %s\n", ctx->expectedService);
    }
    printf_a_ignorar3(output, "== flags: 0x%08x\n", ctx->flags);
    printf_a_ignorar3(output, "== flags2: 0x%08x\n", ctx->flags2);

    printf_a_ignorar2(output, "== Key Info Read Ctx:\n");
    xmlSecKeyInfoCtxDebugDump(&(ctx->keyInfoReadCtx), output);
    
    printf_a_ignorar2(output, "== Key Info Write Ctx:\n");
    xmlSecKeyInfoCtxDebugDump(&(ctx->keyInfoWriteCtx), output);

    if(xmlSecPtrListGetSize(&(ctx->enabledRespondWithIds)) > 0) {
	printf_a_ignorar2(output, "== Enabled RespondWith: ");
	xmlSecTransformIdListDebugDump(&(ctx->enabledRespondWithIds), output);
    } else {
	printf_a_ignorar2(output, "== Enabled RespondWith: all\n");
    }

    if(xmlSecPtrListGetSize(&(ctx->enabledServerRequestIds)) > 0) {
	printf_a_ignorar2(output, "== Enabled ServerRequest: ");
	xmlSecTransformIdListDebugDump(&(ctx->enabledServerRequestIds), output);
    } else {
	printf_a_ignorar2(output, "== Enabled ServerRequest: all\n");
    }

    printf_a_ignorar2(output, "== RespondWith List:\n");
    xmlSecPtrListDebugDump(&(ctx->respWithList), output);

    printf_a_ignorar2(output, "== Keys:\n");
    xmlSecPtrListDebugDump(&(ctx->keys), output);
    
    if(ctx->compoundRequestContexts != NULL) {
        printf_a_ignorar2(output, "== Compound Request:\n");
...
xmlSecXkmsServerCtxDebugXmlDump(xmlSecXkmsServerCtxPtr ctx, FILE* output) {
    ...
    printf_a_ignorar3(output, "<XkmsServerRequestContext name=\"%s\">\n",
	    (ctx->requestId != xmlSecXkmsServerRequestIdUnknown && 
	     xmlSecXkmsServerRequestKlassGetName(ctx->requestId)) ? 
		xmlSecXkmsServerRequestKlassGetName(ctx->requestId) :
		BAD_CAST "NULL");
    ...
    printf_a_ignorar3(output, "<Id>%s</Id>\n", 
		(ctx->id) ? ctx->id : BAD_CAST "");
    printf_a_ignorar3(output, "<Service>%s</Service>\n", 
		(ctx->service) ? ctx->service : BAD_CAST "");
    printf_a_ignorar3(output, "<Nonce>%s</Nonce>\n", 
		(ctx->nonce) ? ctx->nonce : BAD_CAST "");
    printf_a_ignorar3(output, "<OriginalRequestId>%s</OriginalRequestId>\n", 
		(ctx->originalRequestId) ? ctx->originalRequestId : BAD_CAST "");
    printf_a_ignorar3(output, "<PendingNotificationMechanism>%s</PendingNotificationMechanism>\n", 
		(ctx->pendingNotificationMechanism) ? 
		    ctx->pendingNotificationMechanism : 
		    BAD_CAST "");
    printf_a_ignorar3(output, "<PendingNotificationIdentifier>%s</PendingNotificationIdentifier>\n", 
		(ctx->pendingNotificationIdentifier) ? 
		    ctx->pendingNotificationIdentifier : 
		    BAD_CAST "");
    if(ctx->responseLimit != XMLSEC_XKMS_NO_RESPONSE_LIMIT) {
        printf_a_ignorar3(output, "<ResponseLimit>%d</ResponseLimit>\n", ctx->responseLimit);
    }
    ...
    if(ctx->expectedService != NULL) {    
        printf_a_ignorar3(output, "<ExpectedService>%s</ExpectedService>\n", ctx->expectedService);
    }
    printf_a_ignorar3(output, "<Flags>%08x</Flags>\n", ctx->flags);
    printf_a_ignorar3(output, "<Flags2>%08x</Flags2>\n", ctx->flags2);

    printf_a_ignorar2(output, "<KeyInfoReadCtx>\n");
    xmlSecKeyInfoCtxDebugXmlDump(&(ctx->keyInfoReadCtx), output);
    printf_a_ignorar2(output, "</KeyInfoReadCtx>\n");

    printf_a_ignorar2(output, "<KeyInfoWriteCtx>\n");
    xmlSecKeyInfoCtxDebugXmlDump(&(ctx->keyInfoWriteCtx), output);
    printf_a_ignorar2(output, "</KeyInfoWriteCtx>\n");

    if(xmlSecPtrListGetSize(&(ctx->enabledRespondWithIds)) > 0) {
	printf_a_ignorar2(output, "<EnabledRespondWith>\n");
	xmlSecTransformIdListDebugXmlDump(&(ctx->enabledRespondWithIds), output);
	printf_a_ignorar2(output, "</EnabledRespondWith>\n");
    } else {
	printf_a_ignorar2(output, "<EnabledRespondWith>all</EnabledRespondWith>\n");
    }

    if(xmlSecPtrListGetSize(&(ctx->enabledServerRequestIds)) > 0) {
	printf_a_ignorar2(output, "<EnabledServerRequest>\n");
	xmlSecTransformIdListDebugXmlDump(&(ctx->enabledServerRequestIds), output);
	printf_a_ignorar2(output, "</EnabledServerRequest>\n");
    } else {
	printf_a_ignorar2(output, "<EnabledServerRequest>all</EnabledServerRequest>\n");
    }


    printf_a_ignorar2(output, "<RespondWithList>\n");
    ...
    if(ctx->compoundRequestContexts != NULL) {
        printf_a_ignorar2(output, "<CompoundRequest>\n");
        xmlSecPtrListDebugXmlDump(ctx->compoundRequestContexts, output);
        printf_a_ignorar2(output, "</CompoundRequest>\n");
    }

    printf_a_ignorar2(output, "</XkmsServerRequestContext>\n");
}
...
xmlSecXkmsRespondWithDebugDump(xmlSecXkmsRespondWithId id, FILE* output) {
    ...
    printf_a_ignorar4(output, "=== RespondWith: \"%s\" (href=\"%s\")\n", 
...
xmlSecXkmsRespondWithDebugXmlDump(xmlSecXkmsRespondWithId id, FILE* output) {
    ...
    printf_a_ignorar4(output, "<RespondWith href=\"%s\">%s</RespondWith>\n", 
        xmlSecErrorsSafeString(id->valueNs),
        xmlSecErrorsSafeString(id->valueName));
}
...
xmlSecXkmsServerRequestDebugDump(xmlSecXkmsServerRequestId id, FILE* output) {
    ...
    printf_a_ignorar3(output, "=== ServerRequest: %s\n", xmlSecErrorsSafeString(id->name));
}
...
xmlSecXkmsServerRequestDebugXmlDump(xmlSecXkmsServerRequestId id, FILE* output) {
    ...
    printf_a_ignorar3(output, "<ServerRequest>%s</ServerRequest>\n", xmlSecErrorsSafeString(id->name));
}
