...
xmlSecNssKeyDataX509DebugDump(xmlSecKeyDataPtr data, FILE* output) {
    ...
    printf_a_ignorar2(output, "=== X509 Data:\n");
    cert = xmlSecNssKeyDataX509GetKeyCert(data);
    if(cert != NULL) {
	printf_a_ignorar2(output, "==== Key Certificate:\n");
	...
	printf_a_ignorar2(output, "==== Certificate:\n");
	xmlSecNssX509CertDebugDump(cert, output);
    }
...
xmlSecNssKeyDataX509DebugXmlDump(xmlSecKeyDataPtr data, FILE* output) {
    ...
    printf_a_ignorar2(output, "<X509Data>\n");
    cert = xmlSecNssKeyDataX509GetKeyCert(data);
    if(cert != NULL) {
	printf_a_ignorar2(output, "<KeyCertificate>\n");
...
xmlSecNssX509CertDebugDump(CERTCertificate* cert, FILE* output) {
    ...
    for (i = 0; i < sn->len; i++) {
	if (i != sn->len - 1) {
	    printf_a_ignorar3(output, "%02x:", sn->data[i]);
	} else {
	    printf_a_ignorar3(output, "%02x", sn->data[i]);
	}
    }
...
xmlSecNssX509CertDebugXmlDump(CERTCertificate* cert, FILE* output) {
    ...
    printf_a_ignorar2(output, "=== X509 Certificate\n");
    printf_a_ignorar3(output, "==== Subject Name: %s\n", cert->subjectName);
    printf_a_ignorar3(output, "==== Issuer Name: %s\n", cert->issuerName);
    sn = &cert->serialNumber;
    for (i = 0; i < sn->len; i++) {
