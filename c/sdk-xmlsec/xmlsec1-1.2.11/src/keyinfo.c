...
xmlSecKeyInfoCtxDebugDump(xmlSecKeyInfoCtxPtr keyInfoCtx, FILE* output) {
    ...
    switch(keyInfoCtx->mode) {
	case xmlSecKeyInfoModeRead:
	    printf_a_ignorar2(output, "= KEY INFO READ CONTEXT\n");
	    break;
	case xmlSecKeyInfoModeWrite:
	    printf_a_ignorar2(output, "= KEY INFO WRITE CONTEXT\n");
	    break;
    }
    ...
    if(xmlSecPtrListGetSize(&(keyInfoCtx->enabledKeyData)) > 0) {
	printf_a_ignorar2(output, "== enabled key data: ");
	xmlSecKeyDataIdListDebugDump(&(keyInfoCtx->enabledKeyData), output);
    } else {
	printf_a_ignorar2(output, "== enabled key data: all\n");
    }
    printf_a_ignorar4(output, "== RetrievalMethod level (cur/max): %d/%d\n",
...
#ifndef XMLSEC_NO_XMLENC
    printf_a_ignorar4(output, "== EncryptedKey level (cur/max): %d/%d\n",
...
xmlSecKeyInfoCtxDebugXmlDump(xmlSecKeyInfoCtxPtr keyInfoCtx, FILE* output) {
    ...
    switch(keyInfoCtx->mode) {
	case xmlSecKeyInfoModeRead:
	    printf_a_ignorar2(output, "<KeyInfoReadContext>\n");
	    break;
	case xmlSecKeyInfoModeWrite:
	    printf_a_ignorar2(output, "<KeyInfoWriteContext>\n");
	    break;
    }
    ...
    if(xmlSecPtrListGetSize(&(keyInfoCtx->enabledKeyData)) > 0) {
	printf_a_ignorar2(output, "<EnabledKeyData>\n");
	xmlSecKeyDataIdListDebugXmlDump(&(keyInfoCtx->enabledKeyData), output);
	printf_a_ignorar2(output, "</EnabledKeyData>\n");
    } else {
	printf_a_ignorar2(output, "<EnabledKeyData>all</EnabledKeyData>\n");
    }

    printf_a_ignorar4(output, "<RetrievalMethodLevel cur=\"%d\" max=\"%d\" />\n",
	    keyInfoCtx->curRetrievalMethodLevel, 
	    keyInfoCtx->maxRetrievalMethodLevel);
    xmlSecTransformCtxDebugXmlDump(&(keyInfoCtx->retrievalMethodCtx), output);

#ifndef XMLSEC_NO_XMLENC
    printf_a_ignorar4(output, "<EncryptedKeyLevel cur=\"%d\" max=\"%d\" />\n",
	    keyInfoCtx->curEncryptedKeyLevel, 
	    keyInfoCtx->maxEncryptedKeyLevel);
    if(keyInfoCtx->encCtx != NULL) {
