...
xmlSecDSigCtxDebugDump(xmlSecDSigCtxPtr dsigCtx, FILE* output) {
    ...
    if(dsigCtx->operation == xmlSecTransformOperationSign) {    
	printf_a_ignorar2(output, "= SIGNATURE CONTEXT\n");
    } else {
	printf_a_ignorar2(output, "= VERIFICATION CONTEXT\n");
    }
    switch(dsigCtx->status) {
	case xmlSecDSigStatusUnknown:
	    printf_a_ignorar2(output, "== Status: unknown\n");
	    break;
	case xmlSecDSigStatusSucceeded:
	    printf_a_ignorar2(output, "== Status: succeeded\n");
	    break;
	case xmlSecDSigStatusInvalid:
	    printf_a_ignorar2(output, "== Status: invalid\n");
	    break;
    }
...
xmlSecDSigCtxDebugXmlDump(xmlSecDSigCtxPtr dsigCtx, FILE* output) {
    ...
    if(dsigCtx->operation == xmlSecTransformOperationSign) {    
	printf_a_ignorar2(output, "<SignatureContext \n");
    } else {
	printf_a_ignorar2(output, "<VerificationContext \n");
    }
    switch(dsigCtx->status) {
	case xmlSecDSigStatusUnknown:
	    printf_a_ignorar2(output, "status=\"unknown\" >\n");
	...
	printf_a_ignorar2(output, "<Result>");
	fwrite(xmlSecBufferGetData(dsigCtx->result), 
	       xmlSecBufferGetSize(dsigCtx->result), 
	       1, output);
	printf_a_ignorar2(output, "</Result>\n");
	...
	printf_a_ignorar2(output, "<PreSignedData>");
	fwrite(xmlSecBufferGetData(xmlSecDSigCtxGetPreSignBuffer(dsigCtx)), 
	       xmlSecBufferGetSize(xmlSecDSigCtxGetPreSignBuffer(dsigCtx)), 
	       1, output);
	printf_a_ignorar2(output, "</PreSignedData>\n");       
...
xmlSecDSigReferenceCtxDebugDump(xmlSecDSigReferenceCtxPtr dsigRefCtx, FILE* output) {
    ...
    if(dsigRefCtx->dsigCtx->operation == xmlSecTransformOperationSign) {    
	printf_a_ignorar2(output, "= REFERENCE CALCULATION CONTEXT\n");
    } else {
	printf_a_ignorar2(output, "= REFERENCE VERIFICATION CONTEXT\n");
    }
    ...
    if(dsigRefCtx->uri != NULL) {
	printf_a_ignorar3(output, "== URI: \"%s\"\n", dsigRefCtx->uri);
    }
    if(dsigRefCtx->type != NULL) {
	printf_a_ignorar3(output, "== Type: \"%s\"\n", dsigRefCtx->type);
    }

    printf_a_ignorar2(output, "== Reference Transform Ctx:\n");
    xmlSecTransformCtxDebugDump(&(dsigRefCtx->transformCtx), output);

    if(dsigRefCtx->digestMethod != NULL) {
        printf_a_ignorar2(output, "== Digest Method:\n");
	...
	printf_a_ignorar2(output, "== PreDigest data - start buffer:\n");
...
xmlSecDSigReferenceCtxDebugXmlDump(xmlSecDSigReferenceCtxPtr dsigRefCtx, FILE* output) {
    ...
    if(dsigRefCtx->dsigCtx->operation == xmlSecTransformOperationSign) {    
	printf_a_ignorar2(output, "<ReferenceCalculationContext ");
    } else {
	printf_a_ignorar2(output, "<ReferenceVerificationContext ");
    }
    switch(dsigRefCtx->status) {
	case xmlSecDSigStatusUnknown:
	    printf_a_ignorar2(output, "status=\"unknown\" >\n");
    ...
    if(dsigRefCtx->uri != NULL) {
	printf_a_ignorar3(output, "<URI>%s</URI>\n", dsigRefCtx->uri);
    }
    if(dsigRefCtx->type != NULL) {
	printf_a_ignorar3(output, "<Type>%s</Type>\n", dsigRefCtx->type);
    }

    printf_a_ignorar2(output, "<ReferenceTransformCtx>\n");
    xmlSecTransformCtxDebugXmlDump(&(dsigRefCtx->transformCtx), output);
    printf_a_ignorar2(output, "</ReferenceTransformCtx>\n");

    if(dsigRefCtx->digestMethod != NULL) {
        printf_a_ignorar2(output, "<DigestMethod>\n");
	...
	printf_a_ignorar2(output, "<PreDigestData>");
	fwrite(xmlSecBufferGetData(xmlSecDSigReferenceCtxGetPreDigestBuffer(dsigRefCtx)), 
	       xmlSecBufferGetSize(xmlSecDSigReferenceCtxGetPreDigestBuffer(dsigRefCtx)), 
	       1, output);
	printf_a_ignorar2(output, "</PreDigestData>\n");       
