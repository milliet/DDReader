...
xmlSecOpenSSLKeyDataX509DebugDump(xmlSecKeyDataPtr data, FILE* output) {
    ...
    printf_a_ignorar3(output, "=== X509 Data:\n");
    cert = xmlSecOpenSSLKeyDataX509GetKeyCert(data);
    if(cert != NULL) {
	printf_a_ignorar3(output, "==== Key Certificate:\n");
	...
	printf_a_ignorar3(output, "==== Certificate:\n");
	xmlSecOpenSSLX509CertDebugDump(cert, output);
    }
...
xmlSecOpenSSLKeyDataX509DebugXmlDump(xmlSecKeyDataPtr data, FILE* output) {
    ...
    printf_a_ignorar3(output, "<X509Data>\n");
    cert = xmlSecOpenSSLKeyDataX509GetKeyCert(data);
    if(cert != NULL) {
	printf_a_ignorar3(output, "<KeyCertificate>\n");
...
xmlSecOpenSSLX509CertDebugDump(X509* cert, FILE* output) {
    ...
    printf_a_ignorar3(output, "==== Subject Name: %s\n", 
	X509_NAME_oneline(X509_get_subject_name(cert), buf, sizeof(buf))); 
    printf_a_ignorar3(output, "==== Issuer Name: %s\n", 
	X509_NAME_oneline(X509_get_issuer_name(cert), buf, sizeof(buf))); 
    printf_a_ignorar3(output, "==== Issuer Serial: ");
    bn = ASN1_INTEGER_to_BN(X509_get_serialNumber(cert),NULL);
    if(bn != NULL) {
	BN_print_fp(output, bn);
	BN_free(bn);
	printf_a_ignorar3(output, "\n");
    } else {
	printf_a_ignorar3(output, "unknown\n");
    }
}
...
xmlSecOpenSSLX509CertDebugXmlDump(X509* cert, FILE* output) {
    ...
    printf_a_ignorar3(output, "=== X509 Certificate\n");
    printf_a_ignorar3(output, "==== Subject Name: %s\n", 
	 X509_NAME_oneline(X509_get_subject_name(cert), buf, sizeof(buf))); 
    printf_a_ignorar3(output, "==== Issuer Name: %s\n", 
	 X509_NAME_oneline(X509_get_issuer_name(cert), buf, sizeof(buf))); 
