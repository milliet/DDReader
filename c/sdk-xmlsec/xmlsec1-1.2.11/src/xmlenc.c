...
xmlSecEncCtxDebugDump(xmlSecEncCtxPtr encCtx, FILE* output) {
	...
	case xmlEncCtxModeEncryptedData:
		...
		printf_a_ignorar2(output, "= DATA ENCRYPTION CONTEXT\n");
	    } else {
		printf_a_ignorar2(output, "= DATA DECRYPTION CONTEXT\n");
	    }
	    break;
	case xmlEncCtxModeEncryptedKey:
	    if(encCtx->operation == xmlSecTransformOperationEncrypt) {    
		printf_a_ignorar2(output, "= KEY ENCRYPTION CONTEXT\n");
	    } else {
		printf_a_ignorar2(output, "= KEY DECRYPTION CONTEXT\n");
	    }
	    break;
    }
    printf_a_ignorar3(output, "== Status: %s\n",
    ...
    if(encCtx->mimeType != NULL) {
	printf_a_ignorar3(output, "== MimeType: \"%s\"\n", encCtx->mimeType);
    }
    if(encCtx->encoding != NULL) {
	printf_a_ignorar3(output, "== Encoding: \"%s\"\n", encCtx->encoding);
    }
    if(encCtx->recipient != NULL) {
	printf_a_ignorar3(output, "== Recipient: \"%s\"\n", encCtx->recipient);
    }
    if(encCtx->carriedKeyName != NULL) {
	printf_a_ignorar3(output, "== CarriedKeyName: \"%s\"\n", encCtx->carriedKeyName);
    }
    ...
    printf_a_ignorar2(output, "== Encryption Transform Ctx:\n");
    xmlSecTransformCtxDebugDump(&(encCtx->transformCtx), output);

    if(encCtx->encMethod != NULL) {
        printf_a_ignorar2(output, "== Encryption Method:\n");
	xmlSecTransformDebugDump(encCtx->encMethod, output);
    }

    if(encCtx->encKey != NULL) {
        printf_a_ignorar2(output, "== Encryption Key:\n");
...
xmlSecEncCtxDebugXmlDump(xmlSecEncCtxPtr encCtx, FILE* output) {
	...
	case xmlEncCtxModeEncryptedData:
	    if(encCtx->operation == xmlSecTransformOperationEncrypt) {    
		printf_a_ignorar2(output, "<DataEncryptionContext ");
	    } else {
		printf_a_ignorar2(output, "<DataDecryptionContext ");
	    }
	    break;
	case xmlEncCtxModeEncryptedKey:
	    if(encCtx->operation == xmlSecTransformOperationEncrypt) {    
		printf_a_ignorar2(output, "<KeyEncryptionContext ");
	    } else {
		printf_a_ignorar2(output, "<KeyDecryptionContext ");
	    }
	    break;
    }
    printf_a_ignorar3(output, "status=\"%s\" >\n", (encCtx->resultReplaced) ? "replaced" : "not-replaced" );
    ...
    if(encCtx->mimeType != NULL) {
	printf_a_ignorar3(output, "<MimeType>%s</MimeType>\n", encCtx->mimeType);
    }
    if(encCtx->encoding != NULL) {
	printf_a_ignorar3(output, "<Encoding>%s</Encoding>\n", encCtx->encoding);
    }
    if(encCtx->recipient != NULL) {
	printf_a_ignorar3(output, "<Recipient>%s</Recipient>\n", encCtx->recipient);
    }
    if(encCtx->carriedKeyName != NULL) {
	printf_a_ignorar3(output, "<CarriedKeyName>%s</CarriedKeyName>\n", encCtx->carriedKeyName);
    }
    ...
    printf_a_ignorar2(output, "<EncryptionTransformCtx>\n");
    xmlSecTransformCtxDebugXmlDump(&(encCtx->transformCtx), output);
    printf_a_ignorar2(output, "</EncryptionTransformCtx>\n");

    if(encCtx->encMethod != NULL) {
        printf_a_ignorar2(output, "<EncryptionMethod>\n");
	xmlSecTransformDebugXmlDump(encCtx->encMethod, output);
        printf_a_ignorar2(output, "</EncryptionMethod>\n");
    }

    if(encCtx->encKey != NULL) {
        printf_a_ignorar2(output, "<EncryptionKey>\n");
