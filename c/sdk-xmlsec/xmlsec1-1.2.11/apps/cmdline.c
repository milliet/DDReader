...
xmlSecAppCmdLineParamGetString(xmlSecAppCmdLineParamPtr param) {
	...
	printf_a_ignorar3(stderr, "Error: parameter \"%s\" is not string.\n", param->fullName);
	return(NULL);
    }
...
xmlSecAppCmdLineParamGetStringList(xmlSecAppCmdLineParamPtr param) {
	...
	printf_a_ignorar3(stderr, "Error: parameter \"%s\" is not string list.\n", param->fullName);
	return(NULL);
    }
...
xmlSecAppCmdLineParamGetInt(xmlSecAppCmdLineParamPtr param, int def) {
	...
	printf_a_ignorar3(stderr, "Error: parameter \"%s\" is not integer.\n", param->fullName);
	return(def);
    }
...
xmlSecAppCmdLineParamGetTime(xmlSecAppCmdLineParamPtr param, time_t def) {
	...
	printf_a_ignorar3(stderr, "Error: parameter \"%s\" is not time.\n", param->fullName);
	return(def);
    }
...
xmlSecAppCmdLineParamsListParse(xmlSecAppCmdLineParamPtr* params,
	...
	if(param == NULL) {
	    printf_a_ignorar3(stderr, "Error: parameter \"%s\" is not supported or the requested\nfeature might have been disabled during compilation.\n", argv[pos]);
	    return(-1);
	}
	
	ret = xmlSecAppCmdLineParamRead(param, argv, argc, pos);
	if(ret < pos) {
	    printf_a_ignorar3(stderr, "Error: failed to parse parameter \"%s\".\n", argv[pos]);
	    return(-1);
	}
	pos = ret + 1;
    }
	...
	if(argv[ii][0] == '-') {
	    printf_a_ignorar3(stderr, "Error: filename is expected instead of parameter \"%s\".\n", argv[ii]);
	    return(-1);
...
xmlSecAppCmdLineValueCreate(xmlSecAppCmdLineParamPtr param, int pos) {
    ...
    if(value == NULL) {
	printf_a_ignorar3(stderr, "Error: malloc failed (%d bytes).\n", sizeof(xmlSecAppCmdLineValue));
	return(NULL);
    }
...
xmlSecAppCmdLineParamRead(xmlSecAppCmdLineParamPtr param, const char** argv, int argc, int pos) {
	...
	printf_a_ignorar3(stderr, "Error: only one parameter \"%s\" is allowed.\n", argv[pos]);
	return(-1);
    }
    ...
    if(value == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to create value for parameter \"%s\".\n", argv[pos]);
	return(-1);
    }
    if(prev != NULL) {
	...
	case xmlSecAppCmdLineParamTypeString:
	    if(pos + 1 >= argc) {
	        printf_a_ignorar3(stderr, "Error: string argument expected for parameter \"%s\".\n", argv[pos]);
	        return(-1);
	    }
	...
	case xmlSecAppCmdLineParamTypeStringList:
	    if(pos + 1 >= argc) {
	        printf_a_ignorar3(stderr, "Error: string list argument expected for parameter \"%s\".\n", argv[pos]);
	        return(-1);
	    }
	    value->strValue = argv[++pos];
	    buf = (char*)malloc(strlen(value->strValue) + 2);
	    if(buf == NULL) {
	        printf_a_ignorar3(stderr, "Error: failed to allocate memory (%d bytes).\n", strlen(value->strValue) + 2);
	        return(-1);
	    }
	...
	case xmlSecAppCmdLineParamTypeNumber:
	    if(pos + 1 >= argc) {
	        printf_a_ignorar3(stderr, "Error: integer argument expected for parameter \"%s\".\n", argv[pos]);
	        return(-1);
	    }    
	    value->strValue = argv[++pos];
	    if(sscanf(value->strValue, "%d", &(value->intValue)) != 1) {
	        printf_a_ignorar3(stderr, "Error: integer argument \"%s\" is invalid.\n", value->strValue);
	        return(-1);
	    }    
	    break;
	case xmlSecAppCmdLineParamTypeTime:
	    if(pos + 1 >= argc) {
	        printf_a_ignorar3(stderr, "Error: time argument expected for parameter \"%s\".\n", argv[pos]);
	        return(-1);
	    }
	    value->strValue = argv[++pos];
	    if(xmlSecAppCmdLineTimeParamRead(value->strValue, &(value->timeValue)) < 0) {
	        printf_a_ignorar3(stderr, "Error: time argument \"%s\" is invalid, expected format is \"YYYY-MM-DD HH:MM:SS\").\n", value->strValue);
	        return(-1);
	    }    
	    break;
    }
