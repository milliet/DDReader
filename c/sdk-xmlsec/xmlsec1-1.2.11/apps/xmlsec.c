...
int main(int argc, const char **argv) {
	...
	printf_a_ignorar3(stderr, "Error: unknown command \"%s\"\n", argv[1]);
	xmlSecAppPrintUsage();
	goto fail;
    }
    ...
    if(pos < 0) {
	printf_a_ignorar3(stderr, "Error: invalid parameters\n");
	xmlSecAppPrintUsage();
	goto fail;
    }
	...
	case xmlSecAppCommandXkmsServerRequest:
	    if(pos >= argc) {
		printf_a_ignorar3(stderr, "Error: <file> parameter is requried for this command\n");
		xmlSecAppPrintUsage();
		goto fail;
	    }
	    break;
    ...
    if(xmlSecAppInit() < 0) {
	printf_a_ignorar3(stderr, "Error: initialization failed\n");
	xmlSecAppPrintUsage();
	goto fail;
    }    
	...
	printf_a_ignorar3(stderr, "Error: keys manager creation failed\n");
	xmlSecAppPrintUsage();
	goto fail;
    }
	...
	case xmlSecAppCommandCheckKeyData:
	    for(i = pos; i < argc; ++i) {
            if(xmlSecAppCheckKeyData(argv[i]) < 0) {
                printf_a_ignorar3(stderr, "Error: key data \"%s\" not found\n", argv[i]);
                goto fail;
            } else {
                printf_a_ignorar3(stdout, "Key data \"%s\" found\n", argv[i]);
            }
	    }
	    break;
	...
	case xmlSecAppCommandCheckTransforms:
	    for(i = pos; i < argc; ++i) {
            if(xmlSecAppCheckTransform(argv[i]) < 0) {
                printf_a_ignorar3(stderr, "Error: transform \"%s\" not found\n", argv[i]);
                goto fail;
            } else {
                printf_a_ignorar3(stdout, "Transforms \"%s\" found\n", argv[i]);
            }
	    }
	    break;	    
	case xmlSecAppCommandKeys:
	    for(i = pos; i < argc; ++i) {
		    ...
		    printf_a_ignorar3(stderr, "Error: failed to save keys to file \"%s\"\n", argv[i]);
		    goto fail;
		}
	    }
	    break;
	...
	case xmlSecAppCommandSign:
	    for(i = pos; i < argc; ++i) {
		    ...
		    printf_a_ignorar3(stderr, "Error: failed to sign file \"%s\"\n", argv[i]);
		    goto fail;
		}
	    }
	    break;
	case xmlSecAppCommandVerify:
	    for(i = pos; i < argc; ++i) {
		    ...
		    printf_a_ignorar3(stderr, "Error: failed to verify file \"%s\"\n", argv[i]);
		    goto fail;
		}
	    }
	    break;
	...
	case xmlSecAppCommandSignTmpl:
		...
		printf_a_ignorar3(stderr, "Error: failed to create and sign template\n");
		goto fail;
	    }
	    break;
#endif /* XMLSEC_NO_TMPL_TEST */
	...
	case xmlSecAppCommandEncrypt:
	    for(i = pos; i < argc; ++i) {
		    ...
		    printf_a_ignorar3(stderr, "Error: failed to encrypt file with template \"%s\"\n", argv[i]);
		    goto fail;
		}
	    }
	    break;
	case xmlSecAppCommandDecrypt:
	    for(i = pos; i < argc; ++i) {
		    ...
		    printf_a_ignorar3(stderr, "Error: failed to decrypt file \"%s\"\n", argv[i]);
		    goto fail;
		}
	    }
	    break;
	...
	case xmlSecAppCommandEncryptTmpl:
		...
		printf_a_ignorar3(stderr, "Error: failed to create and encrypt template\n");
		goto fail;
	    }
	    break;
#endif /* XMLSEC_NO_TMPL_TEST */
	...
	case xmlSecAppCommandXkmsServerRequest:
	    for(i = pos; i < argc; ++i) {
		    ...
		    printf_a_ignorar3(stderr, "Error: failed to process XKMS server request from file \"%s\"\n", argv[i]);
		    goto fail;
		}
	    }
	    break;
#endif /* XMLSEC_NO_XKMS */
	default:
	    printf_a_ignorar3(stderr, "Error: invalid command %d\n", command);
	    xmlSecAppPrintUsage();
	    goto fail;
	}
    }
	...
	repeats = xmlSecAppCmdLineParamGetInt(&repeatParam, 1);
        printf_a_ignorar3(stderr, "Executed %d tests in %ld msec\n", repeats, total_time / (CLOCKS_PER_SEC / 1000));    
    }
...
xmlSecAppSignFile(const char* filename) {
	...
	printf_a_ignorar3(stderr, "Error: dsig context initialization failed\n");
	return(-1);
    }

    if(xmlSecAppPrepareDSigCtx(&dsigCtx) < 0) {
	printf_a_ignorar3(stderr, "Error: dsig context preparation failed\n");
	goto done;
    }
    ...
    if(data == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to load template \"%s\"\n", filename);
	goto done;
    }
	...
	if(f == NULL) {
	    printf_a_ignorar3(stderr,"Error: failed to open output file \"%s\"\n",
...
xmlSecAppVerifyFile(const char* filename) {
    ...
    data = xmlSecAppXmlDataCreate(filename, xmlSecNodeSignature, xmlSecDSigNs);
    if(data == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to load document \"%s\"\n", filename);
	goto done;
    }
	...
	switch(dsigCtx.status) {
		...
		printf_a_ignorar3(stderr, "ERROR\n");
		break;
	    case xmlSecDSigStatusSucceeded:
		printf_a_ignorar3(stderr, "OK\n");
		break;
	    case xmlSecDSigStatusInvalid:
		printf_a_ignorar3(stderr, "FAIL\n");
		break;
	}    
	...
	for(i = good = 0; i < size; ++i) {
	    ...
	    if(dsigRefCtx == NULL) {
		printf_a_ignorar3(stderr,"Error: reference ctx is null\n");
		goto done;
	    }
	...
	printf_a_ignorar3(stderr, "SignedInfo References (ok/all): %d/%d\n", good, size);
	...
	printf_a_ignorar3(stderr, "Manifests References (ok/all): %d/%d\n", good, size);
...
xmlSecAppSignTmpl(void) {
    ...
    if(doc == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to create doc\n");
	goto done;
    }
    ...
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to create Signature node\n");
	goto done;
    }
    ...
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to find SignatureMethod node\n");
	goto done;
    }
    if(xmlSecTmplTransformAddHmacOutputLength(cur, 93) < 0) {
	printf_a_ignorar3(stderr, "Error: failed to set hmac length\n");
	goto done;
    }
    ...
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to add Reference node\n");
	goto done;
    }
    
    cur = xmlSecTmplReferenceAddTransform(cur, xmlSecTransformXPath2Id);
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to add XPath transform\n");
	goto done;
    }
    
    if(xmlSecTmplTransformAddXPath2(cur, BAD_CAST "intersect", 
				    BAD_CAST "//*[@Id='object1']", NULL) < 0) {
	printf_a_ignorar3(stderr, "Error: failed to set XPath expression\n");
	goto done;    
    }
    ...
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to add Object node\n");
	goto done;
    }
    ...
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to add KeyInfo node\n");
	goto done;	
    }
    if(xmlSecTmplKeyInfoAddKeyName(cur, NULL) == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to add KeyName node\n");
	goto done;	
    }
...
xmlSecAppPrepareDSigCtx(xmlSecDSigCtxPtr dsigCtx) {
    if(dsigCtx == NULL) {
	printf_a_ignorar3(stderr, "Error: dsig context is null\n");
	return(-1);
    }

    /* set key info params */
    if(xmlSecAppPrepareKeyInfoReadCtx(&(dsigCtx->keyInfoReadCtx)) < 0) {
	printf_a_ignorar3(stderr, "Error: failed to prepare key info context\n");
	return(-1);
    }
	...
	if(dsigCtx->signKey == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to generate a session key \"%s\"\n",
	...
	if(dsigCtx->enabledReferenceUris == xmlSecTransformUriTypeNone) {
	    printf_a_ignorar3(stderr, "Error: failed to parse \"%s\"\n",
		    xmlSecAppCmdLineParamGetStringList(&enabledRefUrisParam));
	    return(-1);
	}
    }
...
xmlSecAppEncryptFile(const char* filename) {
	...
	printf_a_ignorar3(stderr, "Error: enc context initialization failed\n");
	return(-1);
    }
    if(xmlSecAppPrepareEncCtx(&encCtx) < 0) {
	printf_a_ignorar3(stderr, "Error: enc context preparation failed\n");
	goto done;
    }
    ...
    if(doc == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to parse xml file \"%s\"\n", 
		filename);
	goto done;
    }
    startTmplNode = xmlSecFindNode(xmlDocGetRootElement(doc), xmlSecNodeEncryptedData, xmlSecEncNs);
    if(startTmplNode == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to find default node with name=\"%s\"\n", 
	...
	if(xmlSecEncCtxUriEncrypt(&encCtx, startTmplNode, BAD_CAST xmlSecAppCmdLineParamGetString(&binaryDataParam)) < 0) {
	    printf_a_ignorar3(stderr, "Error: failed to encrypt file \"%s\"\n", 
	...
	if(data == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to load file \"%s\"\n", 
	...
	if(xmlSecEncCtxXmlEncrypt(&encCtx, startTmplNode, data->startNode) < 0) {
	    printf_a_ignorar3(stderr, "Error: failed to encrypt xml file \"%s\"\n", 
		    xmlSecAppCmdLineParamGetString(&xmlDataParam));
	    goto done;
	}
	total_time += clock() - start_time;    
    } else {
	printf_a_ignorar3(stderr, "Error: encryption data not specified (use \"--xml\" or \"--binary\" options)\n");
	goto done;
    }
...
xmlSecAppDecryptFile(const char* filename) {
	...
	printf_a_ignorar3(stderr, "Error: failed to decrypt file\n");
	goto done;
    }
    total_time += clock() - start_time;    
...
xmlSecAppEncryptTmpl(void) {
    ...
    cur = xmlSecTmplEncDataCreate(doc, xmlSecTransformDes3CbcId, 
				  NULL, NULL, NULL, NULL);
    if(cur == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to encryption template\n");
	goto done;	
    }
    xmlDocSetRootElement(doc, cur);
	...
	printf_a_ignorar3(stderr, "Error: failed to encrypt data\n");
	goto done;	
    }
    total_time += clock() - start_time;    
...
xmlSecAppPrepareEncCtx(xmlSecEncCtxPtr encCtx) {    
    if(encCtx == NULL) {
	printf_a_ignorar3(stderr, "Error: enc context is null\n");
	return(-1);
    }
...
xmlSecAppXkmsServerProcess(const char* filename) {
	...
	printf_a_ignorar3(stderr, "Error: XKMS server context initialization failed\n");
	return(-1);
    }
    if(xmlSecAppPrepareXkmsServerCtx(&xkmsServerCtx) < 0) {
	printf_a_ignorar3(stderr, "Error: XKMS server context preparation failed\n");
	goto done;
    }
	...
	if(format == xmlSecXkmsServerFormatUnknown) {
	    printf_a_ignorar3(stderr, "Error: unknown format \"%s\"\n", 
    ...
    if(data == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to load request from file \"%s\"\n", filename);
	goto done;
    }
    ...
    if(result == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to process xkms server request\n");
	goto done;
    }
    total_time += clock() - start_time;    
...
xmlSecAppPrepareXkmsServerCtx(xmlSecXkmsServerCtxPtr xkmsServerCtx) {    
	...
	printf_a_ignorar3(stderr, "Error: XKMS context is null\n");
	return(-1);
    }
	...
	if(xkmsServerCtx->expectedService == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to duplicate string \"%s\"\n", 
...
xmlSecAppListKeyData(void) {
    printf_a_ignorar3(stdout, "Registered key data klasses:\n");
...
xmlSecAppListTransforms(void) {
    printf_a_ignorar3(stdout, "Registered transform klasses:\n");
...
xmlSecAppPrepareKeyInfoReadCtx(xmlSecKeyInfoCtxPtr keyInfoCtx) {
    ...
    if(keyInfoCtx == NULL) {
	printf_a_ignorar3(stderr, "Error: key info context is null\n");
	return(-1);
    }
	...
	if(value->strListValue == NULL) {
	    printf_a_ignorar3(stderr, "Error: invalid value for option \"%s\".\n", 
		...
		if(dataId == xmlSecKeyDataIdUnknown) {
		    printf_a_ignorar3(stderr, "Error: key data \"%s\" is unknown.\n", p);
		    return(-1);
		}
		ret = xmlSecPtrListAdd(&(keyInfoCtx->enabledKeyData), (const xmlSecPtr)dataId);
		if(ret < 0) {
		    printf_a_ignorar3(stderr, "Error: failed to enable key data \"%s\".\n", p);
		    return(-1);
		}
	    }
	}
...
xmlSecAppLoadKeys(void) {
    ...
    if(gKeysMngr != NULL) {
	printf_a_ignorar3(stderr, "Error: keys manager already initialized.\n");
	return(-1);	
    }    
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to generate key \"%s\".\n", value->strValue);
	    return(-1);
	}	
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load xml keys file \"%s\".\n", value->strValue);
	    return(-1);
	}	
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load private key from \"%s\".\n", 
		    value->strListValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    return(-1);
		    ...
		    value->paramNameValue,
		    xmlSecKeyDataFormatPem) < 0) {
	    printf_a_ignorar3(stderr, "Error: failed to load public key from \"%s\".\n", 
		    value->strListValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load aes key from \"%s\".\n",
		    value->strValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load des key from \"%s\".\n",
		    value->strValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load hmac key from \"%s\".\n",
		    value->strValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load pkcs12 key from \"%s\".\n",
		    value->strValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load trusted cert from \"%s\".\n",
		    value->strValue);
	    return(-1);
	}
    }
	...
	if(value->strValue == NULL) {
	    ...
	    printf_a_ignorar3(stderr, "Error: failed to load untrusted cert from \"%s\".\n",
		    value->strValue);
	    return(-1);
	}
    }
...
xmlSecAppInit(void) {
    ...
    /* Init xmlsec */
    if(xmlSecInit() < 0) {
	printf_a_ignorar3(stderr, "Error: xmlsec intialization failed.\n");
	return(-1);
    }
	...
	printf_a_ignorar3(stderr, "Error: unable to load xmlsec-%s library. Make sure that you have\n"
	...
	printf_a_ignorar3(stderr, "Error: xmlsec crypto intialization failed.\n");
	return(-1);
    }
    return(0);
}
...
xmlSecAppShutdown(void) {
	...
	printf_a_ignorar3(stderr, "Error: xmlsec crypto shutdown failed.\n");
    }
    
    /* Shutdown xmlsec */
    if(xmlSecShutdown() < 0) {
	printf_a_ignorar3(stderr, "Error: xmlsec shutdown failed.\n");
    }
...
xmlSecAppXmlDataCreate(const char* filename, const xmlChar* defStartNodeName, const xmlChar* defStartNodeNs) {
    ...
    if(filename == NULL) {
	printf_a_ignorar3(stderr, "Error: xml filename is null\n");
	return(NULL);
    }
    ...
    if(data == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to create xml data\n");
	return(NULL);
    }
	...
	if(data->dtd == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to parse dtd file \"%s\"\n", 
	...
	if(value->strValue == NULL) {
		...
		if(xmlSecAppAddIDAttr(cur, attrName, nodeName, nsHref) < 0) {
		    printf_a_ignorar3(stderr, "Error: failed to add ID attribute \"%s\" for node \"%s\"\n", attrName, value->strValue);
		    xmlFree(buf);
	    	    xmlSecAppXmlDataDestroy(data);
		    return(NULL);    
		}
	...
	if(attr == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to find node with id=\"%s\"\n", 
	...
	if(buf == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to duplicate node \"%s\"\n", 
	...
	if(cur == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to find node with name=\"%s\"\n", 
		    name);
	    xmlFree(buf);
	    xmlSecAppXmlDataDestroy(data);
	    return(NULL);    
	...
	if(rootNode == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to find root node\n"); 
	    xmlSecAppXmlDataDestroy(data);
	    return(NULL);    
	}
	...
	if(ctx == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to create xpath context\n");
	    xmlSecAppXmlDataDestroy(data);
	    return(NULL);    
	}
	...
	for(ns = rootNode->nsDef; ns != NULL; ns = ns->next) {
		...
		if(ret != 0) {
		    printf_a_ignorar3(stderr, "Error: failed to register namespace \"%s\"\n", ns->prefix);
	...
	if(obj == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to evaluate xpath expression\n");
	    xmlXPathFreeContext(ctx);
	    xmlSecAppXmlDataDestroy(data);
	    return(NULL);    
	}
	...
	if((obj->nodesetval == NULL) || (obj->nodesetval->nodeNr != 1)) {
	    printf_a_ignorar3(stderr, "Error: xpath expression evaluation does not return a single node as expected\n");
	...
	if(cur == NULL) {
	    printf_a_ignorar3(stderr, "Error: failed to get root element\n"); 
	    xmlSecAppXmlDataDestroy(data);
	    return(NULL);    
	}
    }
...
xmlSecAppXmlDataDestroy(xmlSecAppXmlDataPtr data) {
    if(data == NULL) {
	printf_a_ignorar3(stderr, "Error: xml data is null\n");
	return;
    }
    if(data->dtd != NULL) {
	xmlFreeDtd(data->dtd);
...
xmlSecAppPrintHelp(xmlSecAppCommand command, xmlSecAppCmdLineParamTopic topics) {
    ...
    if(topics != 0) {
	printf_a_ignorar3(stdout, "Options:\n");
	xmlSecAppCmdLineParamsListPrint(parameters, topics, stdout);
	printf_a_ignorar3(stdout, "\n");
    }
    printf_a_ignorar3(stdout, "\n%s\n", bugs);
    printf_a_ignorar3(stdout, "%s\n", copyright);
}
...
xmlSecAppGetUriType(const char* string) {
	...
	if(strcmp(string, "empty") == 0) {
	    ...
	    printf_a_ignorar3(stderr, "Error: invalid uri type: \"%s\"\n", string);
...
xmlSecAppOpenFile(const char* filename) {
    ...
    if(file == NULL) {
	printf_a_ignorar3(stderr, "Error: failed to open file \"%s\"\n", filename);
	return(NULL);
    }
...
xmlSecAppWriteResult(xmlDocPtr doc, xmlSecBufferPtr buffer) {
	...
	printf_a_ignorar3(stderr, "Error: both result doc and result buffer are null\n");	
	xmlSecAppCloseFile(f);
	return(-1);
    }    
    xmlSecAppCloseFile(f);
...
xmlSecAppAddIDAttr(xmlNodePtr node, const xmlChar* attrName, const xmlChar* nodeName, const xmlChar* nsHref) {
	...
	printf_a_ignorar3(stderr, "Error: duplicate ID attribute \"%s\"\n", id);	
	xmlFree(id);
	return(-1);
    }
    xmlFree(id);
